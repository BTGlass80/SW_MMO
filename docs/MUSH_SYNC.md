# MUSH Sync Manifest — weekly drift report
...
since the last baseline** — before a human has to eyeball anything.
...
- `docs/design/Guide_*.md` (glob, 24 files as of this writing) — the SW_MUSH
  system design guides that orient every curated import.
- `data/species/*.yaml` (glob, 9 files) — fed `data/species_clone_wars.json`
  in full.
- A curated allowlist of specific `data/*.yaml` and
  ...
  `data/worlds/clone_wars/` — that tree has 100+ files for content
...
- `sw_mush.db` — the live SQLite database, tracked as a single whole-file
  hash.
...
# Weekly check — compares the CURRENT hash of every tracked file against the
...
- `0` — no drift; every tracked file's hash matches the stored baseline.
- `1` — drift detected; see the ADDED / REMOVED / CHANGED sections of the
  report (CHANGED entries include old/new size and the size delta).
- `2` — setup problem: `C:\SW_MUSH` not found at the given `--mush-root`, or
...
baseline — a persistently-locked file shouldn't spam the same "drift" alert
every week.

## What a drift report means

- **ADDED** — a file now exists under a tracked glob (e.g. a new
...
- **REMOVED** — a previously-tracked file is gone (renamed or deleted
  upstream). Check whether this project's existing curated JSON that came
  from it is now stale/orphaned.
- **CHANGED** — a tracked file's content hash differs from the baseline.
...
**A re-port MERGES around prototype-added fields — it never overwrites
...
- `threat_tier`, `boss` — creature/NPC balance tags added in this project.
- `loot_mult` — per-creature loot multiplier tuning (`data/creatures_clone_wars.json`).
- `stun_return_fire` — combat behavior flag added for hostile NPCs.
...
This is out of scope for `tools/mush_sync_manifest.py` itself — it only
...
`tools/check_project.ps1` — no changes were needed there.
