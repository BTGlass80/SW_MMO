...
- The same Cantina geometry/collision runtime stats are now reproduced from `specs/pixel_room_cantina_runtime_adapter_v0.json`.
- The adapter emits source masks, room scenes, collision/nav overlays, socket/path probes, manifest, and review docs without hard-coded room rectangles.
- The adapter now accepts explicit socket roles, facing, and optional actions. Current role counts: cover:2, inspect:1, light:1, prop:2, seat:4, spawn:1, stand:1, transition:1, use:1.
- The geometry/collision cards stayed stable while socket count rose from 12 to 14 and path probes rose from 2 to 3.
...
- Whether the external pixel room runtime adapter repeats cleanly on a second SW_MUSH Cantina room.
