# Living Mos Eisley — The Persistent-World Composition Loop
...
Companion to (and dependent on) the existing backbone designs — this doc does not
re-litigate them, it **binds them together**:

- `docs/WORLD_SIM_DESIGN.md` — the Director/influence/alert/security backbone.
- `docs/FACTION_TERRITORY_DESIGN.md` — claims, cities, the Drop-6D siege machine.
- `docs/PERSISTENCE_DESIGN.md` — what survives restart and the SQLite/Postgres path.
...
awakening, and — as of 2026-07-03 — the live quest system (`quest_model.gd`,
...
- The Director fires world events, but an event is a **headline only** — it changes
...
- Killing hostiles awards CP and (via F1) some territory influence, but **does not
  calm the zone** — the spawn population and threat do not respond.
- The quest board is a **static list** — it does not shift with zone security,
...
This document specifies the two seams that make these compose — a **read seam** (the
Zone Situation blackboard) and a **write seam** (the World Pulse intake) — plus the
...
Grounding: Guide_26 (Director AI) §1 "the system that makes the world feel alive",
§2 influence→alert, §3 world events, §4 the player-feedback loop; Guide_04 (Security
Zones) §7 dynamic overlays; Guide_24 (Encounters & Hazards) §1 the emergent danger
layer; Guide_06 (Economy) §2 mission board, §7 sinks; Guide_10/§5, Guide_11 Director
integration; Guide_20 (Scenes/Plots) §for how events surface into news/RP.

---

## 1. The composition model — one closed loop, two seams
...
```
      ┌──────────────────────────────────────────────────────────────────┐
      │  PLAYERS act on offers                                            │
      │  (combat kills, quest claim/progress, harvest, buy/sell,          │
      │   claim/siege, presence)                                          │
      └───────────────┬──────────────────────────────────────────────────┘
                      │  WRITE SEAM  (§1.2 World Pulse + durable pending-influence)
                      ▼
      ┌──────────────────────────────────────────────────────────────────┐
      │  AUTHORITATIVE TRUTH  (server-owned, persisted; the existing docs) │
      │  faction_zone_state.influence · territory claims/sieges ·         │
      │  live spawn census · economy ledger · quest board cursor ·        │
      │  world_event_instances                                            │
      └───────────────┬──────────────────────────────────────────────────┘
                      │  Director SLOW TICK recomputes (§2), then DERIVES:
                      ▼
      ┌──────────────────────────────────────────────────────────────────┐
      │  ZONE SITUATION BLACKBOARD  (§1.1, derived, READ-ONLY, cached)    │
      │  alert · security_effective · dominant_faction + trend ·          │
      │  controlling_org · threat_level · scarcity_index ·                │
      │  active_event_tags · quest_offer_tags · ambient_mood              │
      └───────────────┬──────────────────────────────────────────────────┘
                      │  READ SEAM  (reactive systems read; NEVER write)
        ┌─────────────┼─────────────┬───────────────┬─────────────────┐
        ▼             ▼             ▼               ▼                 ▼
    SPAWNS(§3.1)  QUESTS(§3.2)  VENDORS(§3.3)  DIALOGUE(§3.4)   AMBIENT/NEWS
    census+table  board filter  price+stock    line selection   (Director)
        └─────────────┴─────────────┴───────────────┴─────────────────┘
                      │  these are the OFFERS players see
                      └──────────────► (back to PLAYERS act)
```
...
influence tables directly — they append typed events to one accumulator the Director
folds deterministically. This is the same discipline the MUSH keeps by routing ALL
influence through one entry point (`adjust_territory_influence`, Guide_11 §2
"Architecture invariant") and ALL combat through one security gate
(`get_effective_security`, Guide_04 §2 "Key architecture invariant").

### 1.1 The read seam — the Zone Situation blackboard
...
`alert_level` / `security_overlay` today, which `PERSISTENCE_DESIGN` §1.B marks
...
```jsonc
{
  ...
  // --- mirrored/derived from faction_zone_state (WORLD_SIM_DESIGN §2.3) ---
  ...
  // --- from territory layer (FACTION_TERRITORY_DESIGN §2) ---
...
### 1.2 The write seam — the World Pulse intake
...
its tick and then clears. This mirrors the durable player→world loop already
...
Proposed shape (`data/schemas/world_pulse.schema.json`) — transient, re-derivable:
...
**Deltas are small by design** (WORLD_SIM_DESIGN §2.2): one player cannot flip a
zone in a session; a handful of players over a week can. The exact plasticity is an
OWNER DECISION (§7.1).

### 1.3 The shared-state ownership contract

To keep authority unambiguous (server owns all truth; clients observe — the
...
| Live spawn census | spawn system (§3.1) | blackboard `threat_level` | Yes (count) |
| Territory claims / sieges | `territory_model` / siege machine | blackboard `controlling_org` | Yes |
| `world_event_instances` | Director event engine (§4) | blackboard `active_event_*` | Yes |
| Quest board cursor / offer set | quest board (§3.2) | clients (offers) | Yes (cursor) |
| Vendor restock timers | vendor system (§3.3) | clients (stock/price) | Yes (timers) |

---

## 2. The world-tick loop (cadence composition)

Everything reactive runs on the **existing slow Director tick** — never the 20 Hz
movement tick (WORLD_SIM_DESIGN §4). The slow tick is owner-configurable (~30 s in
...
|---|---|---|
| **Every slow tick** | Fold World Pulse → influence + census; derive alert/security (exists); **advance world-event state machine (§4)**; **re-derive Zone Situation blackboard**; recompute vendor price multipliers + quest offer set (both pure filters over the blackboard) | cheap, pure |
| **Every Faction Turn** (N slow ticks, ~30 min real) | Full influence recompute + decay-toward-baseline (exists); **world-event trigger roll (§4)**; **spawn population rebalance/regrow (§3.1)**; **vendor restock (§3.3)**; refresh ambient/dialogue variant pools | moderate |
| **Hourly** | Presence influence (exists) | light |
| **Daily** | Influence decay floor (exists); territory yields (exists); **full quest-board rotation (§3.2)** | light |
...
2. **Advance** authoritative sub-systems: event state machine, census regrow, siege
   timers.
3. **Derive** the caches: alert → security overlay → **blackboard** (read seam),
...
The player never sees the tick — they see its *results*: a news line, a shifted
alert badge, thinner spawns, a changed board, a new vendor price (Guide_26 §6 "The
Faction Turn is silent for players").

---

## 3. Reactive content — making the systems talk

Each reactive system is a **pure projection**: `f(authored_content, zone_situation)
→ what the player sees`. The authored content is the immutable seed in `data/`; the
...
the loop between combat (live) and the world (static). Grounding: Guide_24 §1
(emergent danger), Guide_26 §2 (alert drives spawn rates), Guide_11 §2 (kill NPC in
zone → influence).
...
below target across Faction Turns, `threat_level` falls → the alert calms, fewer/
...
- **Pacification (always):** live census −1; `recent_kills` +1. Lowers derived
...
- **Geopolitics (only faction-tagged kills):** killing `underworld_thug`
  (`faction_tag: hutt`) writes Hutt −N to the Pulse; killing a Republic patrol
  writes Republic −N. Wild creatures (`faction_tag: wild`) pacify but do **not**
  shift faction influence. *Whether pacifying wildlife should also nudge the
  "independent/order" axis is an OWNER DECISION — §7.4.*
...
(`creature_spawn_model.gd` "zone-posture bias: dangerous→hostile, calm→non-hostile");
...
`underworld_thug`s. Census drops 11→4; `recent_kills` spikes; `threat_level` 3→1;
Hutt influence ticks down via the Pulse. Next Faction Turn: fewer thug spawns, the
alert eases toward `lax`, the Hutt vendor's contraband thins (§3.3), and Greeshk's
"Fringe Cleanup" quest stops appearing (its `require_faction_min.hutt` no longer
holds — §3.2). Two days later, nobody has been out there; the population regrows,
threat climbs, the quest returns. The desert *reacted*.

### 3.2 Quests reactive to zone security / alert / faction

Grounding: Guide_06 §2 (the board refreshes; jobs scale to context), Guide_04 §9
(lawless-only higher-paying jobs), Guide_26 §3 (`bounty_surge` doubles bounty pay).
...
authored quest. **Absent `availability` ⇒ always offered** (backward-compatible: all
12 current quests keep working untouched). Present ⇒ the board only offers the quest
...
```jsonc
{
  ...
  "reward_dynamic": {                           // OPTIONAL; applied at claim time
    ...
    "cp_bonus_if_security": {"lawless": 1}       // Guide_04 §9 lawless incentive
  }
}
```

The **board offer set** is a pure filter over authored quests × the blackboard,
recomputed each slow tick and rotated daily (Guide_06 §2 refresh cadence). Proposed
...
differently) is an OWNER DECISION — §7.3.

### 3.3 Vendor stock & price reactive to zone state

Grounding: Guide_06 §7 sinks + §8 Bargain, Guide_26 §3 (`trade_boom` +25%,
`merchant_arrival` new goods, `hutt_auction` rare goods), Guide_04 §9 (black-market
...
Director event — E11). This doc makes the **event→price/stock mapping data-driven**
and adds scarcity.
...
```jsonc
{
  ...
  "price_modifiers": {
    ...
    "scarcity_coefficient": 0.3            // final ×= 1 + coefficient * scarcity_index
  },
  "stock_modifiers": {
    ...
    "contraband_when": {                    // gray/black-market gating (OWNER — §7.5)
  ...
  "price_clamp": {"min_mult": 0.7, "max_mult": 1.6},   // anti-exploit bound (§7.2)
  "extra": {}
}
```
...
**bounded** and feeds only price, never availability of essentials — the economy
audit's warning that static multipliers become "a solved game" (Guide_06 §1/§5) is
answered by the `price_clamp` and by scarcity being a small, mean-reverting nudge,
not a runaway. The final price stays: `base × alert × event × (1 + 0.3·scarcity) ×
bargain`, clamped. The **hard price bound is an OWNER DECISION** (§7.2), and any
...
Grounding: Guide_26 §5 (ambient text scoped by zone character), Guide_10 §5 (Director
...
each NPC's authored base lines. Absent ⇒ the NPC speaks its base lines (fully
backward-compatible).
...
```jsonc
{
  ...
  "situational_lines": [
    { "when": {"alert": ["lockdown", "high_alert"]},
      "lines": ["Lockdown's on. Full manifest scan today, citizen — no exceptions."] },
  ...
  "selection": "most_specific_match_then_random",  // #matched-keys wins; ties → seeded random
  "extra": {}
}
```
...
its people** — the cheapest immersion win per unit of work in the whole design.

---

## 4. World events — the effect engine (data + state machine)

Today an event is a headline. This section turns each event into a **data-defined
bundle of effects** the reactive systems in §3 already know how to interpret, plus a
...
Grounding: Guide_26 §3 (the 12-event menu, lifecycle, "opportunities not
obligations", concurrency constraints), §4 (era milestones), WORLD_SIM_DESIGN §2.4
...
trigger conditions and — the new part — their **effects payload**. The Director still
...
```jsonc
{
  ...
  "trigger": {
    ...
    "cooldown_faction_turns": 8,               // same type can't repeat within N (Guide_26 §3)
    "min_gap_minutes": 15
  },
  "duration_faction_turns": {"min": 2, "max": 4},
  "telegraph": true,                            // brief ANNOUNCED phase before ACTIVE (§4.2)
  "effects": {                                  // ← interpreted by §3 systems, all optional
    "influence_per_turn": {"republic": 2, "hutt": -1},
    "security_overlay": "upgrade_one_tier",     // feeds security_gate.gd (contested→secured)
...
— new definitions are content, not code):
...
| **Raid** | `pirate_surge`, `krayt_sighting`, plus additive `faction_raid` | Group content: repel the surge to pacify the zone (§3.1 census spike) |
| **Bounty** | `bounty_surge` | Bounty pay ×2 window; hunt marked targets while it's hot |
...
"Director selects from a fixed menu" safety rule — but because they extend a menu the
...
edit that ledger; it flags the requirement). The far larger structural change — event
**effects becoming data** rather than the current headline-only behavior — is a pure
capability addition and does not diverge from WEG; it realizes Guide_26 §3's intended
...
`world_event_instances` table in PERSISTENCE_DESIGN §3.B):
...
`config` snapshot, FACTION_TERRITORY_DESIGN §6.2).

State machine (slow-tick driven; never client-driven):

```
   trigger roll passes at a Faction Turn AND preconditions hold
 (none) ─────────────────────────────────────────────▶ ANNOUNCED  (telegraph=true)
     │ telegraph=false                                    │  next slow tick: news line + comlink;
     │ (fire immediately)                                 │  effects NOT yet applied
     └───────────────────────────────┐                   ▼
                                      └───────────────▶ ACTIVE  ◀── effects applied every tick
   admin cancel / precondition lost ──▶ CANCELLED           │  (influence/spawn/vendor/quest/dialogue)
   (terminal; effects reset)                                │  expires_at_tick reached
                                                            ▼
                                                        EXPIRING  (one tick: reset overlays/effects,
                                                            │        thin out event-spawned hostiles)
                                                            ▼
                                                        EXPIRED   (archived to director_log / +news)
```

**Concurrency guards** (Guide_26 §3, WORLD_SIM_DESIGN §2.4, all owner-tunable):
≤2 concurrent Director events globally; ≥15 min between Director events; same
...
jails or hard-walls). A `pirate_surge` does not force combat — it makes the deep
...
Unchanged from WORLD_SIM_DESIGN §2.5 and Guide_26 §4 — the Director tracks average
...
## 5. Persistence — the seed + overlay pattern
...
and history** on top. On restart: load seed → apply overlay → re-derive all caches.
...
| **Overlay (durable drift/history)** | per-zone influence drift, `world_event_instances`, `world_milestones`, live spawn **count**, territory claims/sieges, quest-board rotation cursor, vendor restock timers, `world_hooks.pending_zone_influence` on characters | survives restart | SQLite tables (PERSISTENCE_DESIGN §3) |
...
- **Live spawn census per zone** — a small `zone_spawn_census(zone_id, live_count,
...
- **Quest-board cursor per zone** — which authored quests are currently on the board
  and their rotation timer, so a restart resumes the same board rather than
  re-rolling it.
- **Vendor restock timers per zone** — so a mid-cycle restart does not reset the
  economy's supply clock.

**Restart resume specifics** (extending PERSISTENCE_DESIGN §4): an event in `active`
with a future `expires_at_tick` keeps running; one whose expiry passed during
downtime advances to `expiring→expired` on the first tick (its effects are cleanly
reset, not left stuck applied). The blackboard, price map, and offer set are all
**re-derived on the first post-restart tick** from the overlay — never restored from
...
**Whether the galaxy ever resets to baseline** — a hard seasonal wipe of the overlay
back to the seed — is an OWNER DECISION (§7.6). The default is: it never does; drift
is permanent until decay slowly returns it to each zone's authored `baseline`
(already in `zones_clone_wars.json`).

---

## 6. Build order — most "alive" per unit of work
...
models rather than greenfield — noted, because that lowers cost and raises priority.

**Step 1 — Zone Situation blackboard.** `[PURE]` derive the blackboard object
(§1.1) from the already-live `zone_state.gd` + `security_gate.gd` + `territory_model`
...
**Step 2 — Reactive spawns + census (the "clear a zone calms it" loop).** `[PURE]`
...
**Step 3 — Event effects become data.** `[PURE]` `world_event_definition` +
...
`spawn_multiplier`, `influence_per_turn`. *Now events DO things* — a crackdown
actually changes the zone. Unlocks Steps 4–6 to react to events, not just alert.

**Step 4 — Reactive quest board.** `[PURE]` the availability filter + reward_dynamic
...
**Step 5 — Reactive vendor stock/price.** `[PURE]` `vendor_dynamics` modifiers
...
**Step 6 — Reactive NPC dialogue.** `[PURE]` the situational-line selector over the
...
**Step 7 — The composed payoff: full faction-push / raid / shortage events.** No new
seams — just author the `faction_raid` and `supply_shortage` definitions (after their
ledger row, §4.1) and let Steps 1–6 light up together. A single `republic_crackdown`
...
Milestones (§4.3) ride on Step 3 for free once global-scope events work; they are a
...
1. **World plasticity — how fast player action moves a zone.** The magnitudes on the
   ...
   MUSH-faithful slow — "steady but slow change… over hours" (Guide_26 §6), a zone
   needs sustained group effort to flip; (b) responsive — a busy evening noticeably
...
2. **Reactive price bound.** How far may `alert × event × scarcity` move a price
   before Bargain? The economy audit warns static multipliers become "a solved game"
   (Guide_06 §1/§5). *Options:* tight clamp (±15%, cosmetic), medium (±40%, the
   default in §3.3), wide (±100%, real speculation but exploitable). *Recommendation:*
   **medium (0.7–1.6×)** with essentials never removed from stock — meaningful without
   inviting the 120× exploit the audit found.

3. **Quest/vendor gating — hard vs soft.** Should `require_controlling_org_axis` /
   ...
   control matter (Guide_04 §9, Guide_11 endgame); soft gating protects solo and
   ...
   quests** behind influence/territory — so control unlocks *special* content without
   starving anyone.

4. **Does pacifying wildlife have geopolitical weight?** Faction-tagged kills shift
   influence; wild-creature kills currently only pacify (§3.1). *Options:* (a) wild
   kills are purely local threat reduction (recommended — keeps geopolitics about
...
5. **Contraband / gray-market vendor rows.** §3.3's `contraband_when` is authored
   **empty** on purpose. Whether Hutt-dominant/underworld zones can *sell* restricted
   gear over the counter (vs. only via the smuggling loop, Guide_06 §4) is a direct
   lever on how criminal the underworld feels — and couples to the death/loot and PvP
...
6. **Overlay reset cadence.** Does the galaxy ever wipe back to seed (a seasonal
   ...
   permanent (default; the MUSH's months-long "galactic narrative arc", Guide_26 §8);
...
penalty model, and CP progression pace (WORLD_SIM_DESIGN §7, PERSISTENCE_DESIGN §5).
...
## 8. Grounding — MUSH guides drawn from

Read-only inspiration; recast to WEG D6 R&E, server-authoritative, Clone Wars era.

- **Guide_26 (Director AI)** — the whole compose-into-a-living-world thesis: §1 what
  the Director does, §2 influence→alert, §3 the 12-event menu + lifecycle +
  "opportunities not obligations" + concurrency constraints, §4 the player-feedback
  loop + era milestones, §5 ambient text, §6 the silent Faction Turn.
- **Guide_04 (Security Zones)** — §2/§7 dynamic security overlays from influence/
  events (the effective-security composition), §9 lawless incentives (quest/vendor
  reactivity to security).
- **Guide_24 (Encounters & Hazards)** — §1 the emergent danger layer, per-type
  cooldowns and zone caps (spawn pacing), the "world meets you halfway" goal that the
  §3.1 census realizes on the ground.
- **Guide_06 (Economy)** — §2 mission board refresh + context scaling, §3 bounty
  board (`bounty_surge`), §5/§7 the price-exploit warning and credit sinks (the
  §3.3 clamp answers it), §8 Bargain.
- **Guide_10 (Organizations & Factions)** & **Guide_11 (Territory Control)** — the
...
- **Guide_20 (Scenes, Plots & Places)** — the RP layer that world events surface
...
- `zone_situation.schema.json` — the derived read seam (§1.1).
- `world_pulse.schema.json` — the transient write seam (§1.2).
- `spawn_table.schema.json` — reactive spawns + census (§3.1).
- quest `availability` + `reward_dynamic` additive blocks + `quest_offer.schema.json`
  (§3.2).
- `vendor_dynamics.schema.json` — reactive stock/price (§3.3).
- `npc_dialogue_variants.schema.json` — reactive dialogue (§3.4).
- `world_event_definition.schema.json` + `world_event_instance.schema.json` — the
  event effect engine + state machine (§4).

Storage of all durable overlay state: `docs/PERSISTENCE_DESIGN.md` (this doc adds the
`zone_spawn_census`, quest-board cursor, and vendor restock-timer rows in §5).
