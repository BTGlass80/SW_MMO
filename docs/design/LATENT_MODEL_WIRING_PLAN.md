# Latent Model Wiring — code-level execution plan
...
Author: world-sim-designer (DESIGN only — no gameplay GDScript here). Clone Wars era (20 BBY, Mos
...
changes a model's contract — flagged inline where that risk exists.
...
- **d6-rules-engineer** owns: the two seams that touch the pure combat resolver — the armor broken-pool
...
- Combat resolves in ~5s action windows: `network_manager._physics_process` calls
  `_resolve_combat_window()` then `_tick_downed()` every `combat_window_seconds` (~L1983–1987). The
  action window **is** the round unit — poison "per-round" ticks map to per-window ticks.
- The creature-loot hook already fires on a hostile DISABLE inside `_resolve_combat_window` (~L2166–2177):
...
- `_handle_player_death` writes the corpse manifest to `record.world_hooks.corpse` with
  `decay_unix: 0.0` (a placeholder) and `full_loot: PvpRules.is_full_loot(tier)` (~L1599–1605). No
  loot-back RPC exists — `submit_loot_corpse` is genuinely new. The victim's dropped items are already
...
- The player soak pool is Strength (`_pools_from_sheet` L153: `player_soak_pool = parse_pool(strength)`);
  ...
  `ground_combat_model` at the soak build (L311–325 self-defense, L478–491 return-fire), which already
  degrades pips via `ArmorConditionModel.apply_degradation`. **This is where the broken-pool halving belongs.**
- Server owns all RNG (`_server_rng`) and the clock. `TelemetryLog` never touches `Time` itself —
...
## Seam 1 — Harvest (`harvest_model.gd`, DIV-0023): a disabled creature yields a sellable good

### Seam (file · function · anchor)
`network_manager._resolve_combat_window`, the creature-loot block `if tkey != "" and not looted.has(tkey):`
(~L2166–2177), immediately after the existing `_award_credits(shooter, loot_credits)` and before
`arena.remove_hostile_target(tkey)`. The `spawn` dict is already in hand (`arena.hostile_target_spawn(tkey)`).

### Concrete change
1. Gate on `HarvestModel.has_harvest(spawn, _creatures_data)` (most creatures return nothing — the
...
2. Resolve the shooter's field-dress pool from their sheet: the model's governing skill is
   ...
   (untrained → `null` = 0D, which the model coerces). A tiny helper `_field_dress_pool(character_id)`
...
### THE GAP — a harvest good is NOT in the buy catalog and has no price
Harvest goods (`gornt_meat`, `krayt_dragon_pearl`, `acklay_chitin`, …) live only in
`data/creatures_clone_wars.json` and carry **no `cost`**. `_buy_catalog` is weapons+armor only, so the
good is neither buyable nor sellable today. The model deliberately returns a *descriptor* and stops —
"pricing is deferred to the server." Two clean ways to give it value, both inside DIV-0018:

- **Option A — award credits at the point of harvest (mirror salvage).** Add
  `data/harvest_values_clone_wars.json` mapping `good` (or `resource` bucket) → per-unit credit value;
  the server computes `value * quantity * tier_scalar` and grants it through the **existing**
  `_award_credits(shooter, credits)` — byte-for-byte the same path `roll_loot`'s `salvage_credits`
  ...
  good is never buyable, so no buy→sell loop). Lowest risk; ships in the same edit as the roll.
- **Option B — grant a carryable inventory good the vendor buys back.** Append `result.good` to
  ...
  refuse re-buying them — they are never `vendor_stocked`, so `can_buy` already rejects them). Gives the
  good object-permanence for future crafting / quest turn-ins and feeds the **living-world
  `economy_pulse` / `scarcity_index`** (`docs/design/LIVING_WORLD_DESIGN.md` §2/§3.3: "selling floods,
...
**shape** — instant credits vs a carryable resource good — changes what the resource economy can become,
so:

> **OPEN OWNER DECISION (harvest good → value):** A) instant credits at harvest (recommended first cut,
> reuses the salvage path), or B) a carryable inventory resource good sold to vendors (feeds the
> living-world scarcity index, enables crafting/turn-ins later). Not deciding here — presenting A→B as
...
### Edge cases / risks
- Most creatures have no harvest block → `has_harvest` false → no-op (verified against real data: only
  ~15 carry a block). Guard first so the hot loot path stays cheap.
- Gated goods (krayt difficulty 15) fail for untrained field-dressers → `quantity 0` → grant nothing.
  Correct and intended (the skill matters).
- Determinism: the harvest roll consumes `_server_rng` **after** `roll_loot` already did — order is
...
- Only the ONE credited shooter per `looted[tkey]` harvests (dedup already guards multi-shooter double
  credit) — a single kill yields one harvest.

### Smoke to add — `scripts/tests/harvest_wire_smoke.gd` (SceneTree)
...
expected table value × quantity (Option A) or that `sheet.inventory` gained the good (Option B), and
...
## Seam 2 — Creature special attacks (`creature_special_attack_model.gd`, DIV-0024): poison + restraint

### Seam (file · function · anchor)
Two anchors in `network_manager`:
- **Enqueue:** `_resolve_combat_window` envelope loop (~L2156–2184), in the branch where a player
...
- **Apply:** a new `_tick_status()` called right beside `_tick_downed()` in `_physics_process`
  ...
  separate call — `_resolve_combat_window` early-returns on zero intents).
...
### Concrete change
1. On a landed hostile hit, look up the rider: `spawn = arena.hostile_target_spawn(tkey)` →
...
4. **`_tick_status()`** each window, for each poisoned peer: take the next `total` from the queue and
   apply it as damage **through the same wound path a hit uses** — feed it to `WoundLadder.escalate(prior_level,
   ...
   `_resolve_combat_window` does (sev 5 → death, sev 3–4 → downed). Credit the `source_creature`'s
   takedown only once (reuse the `credit_killer` discipline — the creature isn't a peer, so `killer_peer`
...
- Poison ticks route through `WoundLadder.escalate` (G2) so venom **accumulates** up the ladder like any
  hit — it does not overwrite highest-hit-wins.
...
- `0D` paralytic venom (`rock_wart`) yields `total 0` — it applies **no HP**; treat it as a status.
...
`_status_effects[peer].restraint` — a d6-rules-engineer + netcode seam. Recommend restraint be a
...
### Edge cases / risks
- **Highest coupling of the five** — it touches the live wound ladder, the G1/G2 classifier, and (for
...
- Persistence: `_status_effects` is RAM-only (like `_downed`). But combat damage IS persisted via
  `apply_combat` → `sheet.wound_state`, so a logout mid-poison leaves the player at their last applied
  wound (acceptable; the queue simply stops — do NOT try to persist the schedule in v1).
...
### Smoke to add — `scripts/tests/special_attack_wire_smoke.gd` (SceneTree)
Headless server: enqueue a known poison schedule (e.g. `spor_crawler` 5D×3) on a peer, run N
...
routes through the downed→bleed-out/yield path (never a softlock). No `SCRIPT ERROR`.

---

## Seam 3 — Corpse decay + third-party loot (`corpse_decay_model.gd`, DIV-0025)

### Seam (file · function · anchor)
Three anchors in `network_manager`:
- **Stamp the clock:** `_handle_player_death`, the corpse-manifest write (~L1602). `decay_unix` is
  `0.0` today — change it to the server clock `Time.get_unix_time_from_system()`.
...
- **RPC + despawn:** a new `submit_loot_corpse` RPC, and a `_despawn_expired_corpses()` call added to
  the Director-tick block in `_physics_process` (~L1996–2001, beside `_advance_hostiles` / `_save_world_state`).
...
2. **Registry `_corpses`:** `character_id → {zone_id, pos, decay_unix, security_tier}` written when a
   death produces a non-null manifest (skip when `corpse == null`). This lets both the despawn tick and
   the loot RPC find corpses (incl. corpses of players who have since logged off — their record still
   holds the manifest).
3. **`submit_loot_corpse(target_character_id)` (`@rpc("any_peer", "reliable")`):**
   ...
   - On `result.looted`: append `result.items` to the looter's `sheet.inventory` (same append shape as
     ...
     victim record. `result.credits` is always 0 (DIV-0006 credits kept) — never transfer credits.
...
4. **`_despawn_expired_corpses()`:** iterate `_corpses`; for each, if
   ...
   entry. Runs on the Director cadence (coarse is fine — the model's boundary is inclusive).
...
- **Full-loot gate is already enforced by the model** via the manifest's own `full_loot` stamp (lawless
  only). The server does not re-derive it — pass tier and let the model agree with its own stamp.
- **Double-loot race:** two looters in the same window — the server is single-threaded per tick;
  null-the-manifest-then-save inside the RPC handler makes the second loot see `no_corpse`. Safe.
- Looting NEVER mutates the source (the model returns a defensive copy) — the server owns the null-out.
...
### Smoke to add — `scripts/tests/corpse_loot_wire_smoke.gd` (SceneTree)
...
## Seam 4 — Armor broken tier + repair sink (`armor_repair_model.gd`, DIV-0026)

Two independent halves — a **combat-side pool halving** (rules seam) and a **vendor repair RPC**
...
applied to the soak pool (L311–325 self-defense, L478–491 return-fire). This is the single place the pip
...
lives in `ground_combat`/arena `state`, not in the sheet at `_pools_from_sheet` time — pre-scaling in
`_pools_from_sheet` (L153) would not know the live pip. Note the DIV-0026 contract: broken = the
condition FLOOR (−6), not a separate durability=0 axis.
**Risk:** this changes live TTK when armor is broken — re-run `tools/balance_probe.gd` after. It also
...
`submit_sell` (~L1206–1269) — repair is the same vendor interaction class (a credit sink).
**Concrete change:**
...
2. `cost = ArmorRepair.repair_cost(current_pips, ArmorRepair.MAX_QUALITY_PIPS, list_cost)` — a full
...
4. Reply `repair_result.rpc_id(sender, {ok, item_key, cost, credits, quality_pips})`; reasons:
   `unknown_item`, `not_broken`/`no_op` (cost 0 → already at ceiling), `cannot_afford`, `unpriced`
   (list_cost ≤ 0 → model returns cost 0).
...
repair also requires persisting it onto the sheet (a small schema addition — note it, and if it changes
...
### Edge cases / risks
- Unpriced gear (contraband / faction-issued, `cost 0`) → `repair_cost` returns 0 → reject as
...
- The 4a halving and 4b repair are independent — 4b (a pure credit sink, no combat change) is the
  safest to ship first; 4a changes combat math and needs the balance re-probe.

### Smoke to add — `scripts/tests/repair_wire_smoke.gd` (SceneTree)
...
## Seam 5 — Structured telemetry (`telemetry_log.gd`): route six event types from live print sites
...
telemetry failure never breaks the server — the model already degrades to a safe `false` no-op on a bad
open, so no try/guard is needed beyond a null-check on `_telemetry`.

### Reused pieces
`TelemetryLog.new/log_event`, the six existing print sites (leave the prints — telemetry is additive),
`Time` for `ts`. New: one member + six one-line calls.

### Edge cases / risks
- **Safest and most independent of the five** — writes only under `user://`, never touches combat/
...
- It is the **enabler** for tuning the other four (death rate, economy flow, TTK) — this is why it ships
  first.
...
1. **Telemetry (Seam 5)** — FIRST. Fully independent, `user://`-only, degrades safely, and it is the
...
2. **Armor repair RPC (Seam 4b)** — a clean credit sink that reuses the vendor scaffolding + shipped
...
3. **Harvest (Seam 1)** — reuses the live loot hook + `_award_credits` (Option A). Independent of the
   others; gated on the ONE owner decision (good→value). Ships in one edit once that is answered.
4. **Corpse loot (Seam 3)** — self-contained (new RPC + registry + despawn tick); needs only the live
   death path, which exists. Medium surface (a boot-scan for restart continuity) but no combat coupling.
5. **Special attacks (Seam 2) + armor pool-halving (Seam 4a)** — LAST, together. These are the only
...
1. **Harvest good → value conversion (Seam 1):** instant credits (recommended first) vs a carryable
   inventory resource good the vendor buys back (feeds the living-world scarcity index; enables crafting/
   turn-ins). Presented A→B as a sequence; the values themselves are tunable content within DIV-0018.
...
tunable content or fixed by an existing DIV row — no new owner fork.
