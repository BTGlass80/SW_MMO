# Wave F â€” Fresh-Chat Handoff (2026-07-02)
...
## Where we are â€” WAVE F COMPLETE (2026-07-02)
...
- **Pure/rules foundation (S0â€“S5)** â€” committed green (see UNLOCK_SYSTEMS_PLAN).
- **HOT wiring â€” ALL DONE + verified:**
  - **S6** `f1e8660` â€” combat_arena per-player lethal flag + hostile targets (byte-identical sparring when off).
  - **S7â€“S10** `04d39c6` â€” economy: `_award_credits`/`apply_credits`, `submit_vendor_list`/`submit_buy`/`submit_sell`, client shop HUD, `economy_flow_smoke`. 2-proc: 1000â†’buy 750â†’250â†’sell +110â†’360.
  - **S11â€“S13** `e1631ad` â€” Director-tick hostile spawner (lawless+contested) + death/respawn (`_handle_player_death`) + creature loot + `submit_buy_insurance`; `death_flow_smoke`, `--force-hostile`. 2-proc: Merdeth killâ†’respawn, Crab loot, insurance.
  - **S16â€“S19** `0d695dc` â€” Force awakening wiring (signal feeds â†’ Director advance â†’ `apply_completion` flip + client notice); `force_flow_smoke`, `--force-awaken`. 2-proc: forced awaken â†’ phase 5 + notice.
  - **DIV-0019 zone PvP** `7a7e63c`/`57c0b1a`/`5502624` â€” `pvp_rules_model` (lawless-only same-zone `can_fire`), `suppress_return_fire` flag, `resolve_window(seed, pvp_gate)`, submit+resolve gate, casualty/death routing (killer credited, full-loot corpse flag), `--fire-target`/`--fire-nearest`; `pvp_rules_model_smoke` + `pvp_flow_smoke`. 2-proc: kill/respawn in lawless dune_sea; `refused (protected_zone)` in the spaceport.
- **State:** master at `5502624`+ (a parallel session also commits design docs to master â€” scoped commits only).
...
effect; the âˆ’1D post-death DEATH_DEBUFF live; server-global (offline) Force soft-cap tally; consensual
duels in protected zones (see the parallel session's `PVP_CONSENT_DESIGN`).

## Original plan (historical â€” the HOT wiring list, now all DONE)

## Owner decisions (build to these â€” all DECIDED)
1. **Death** = partial loss + insurance, **credits kept**. Numbers (tunable consts, live in the models):
   ...
   respawn at the nearest secured med bay as `wounded` + the existing âˆ’1D death debuff.
...
3. **Force access** = the **SWG "Village"** earned unlock â€” a rare, hidden, multi-phase awakening questline
...
4. **PvP = ZONE-BASED** (NEW, 2026-07-02): **lawless zones are open-PvP**; secured/contested protected.
   This is NOT in the original S0â€“S19 list â€” it needs its own design pass + slices (a PvP target/consent
...
Per UNLOCK_SYSTEMS_PLAN.md, next is **S6** â€” a combat_arena lethal flag (skip the DIV-0016 sparring clamp
for hostile NPCs; sparring behavior byte-identical when off), then S7â€“S13 (shop RPCs â†’ creature spawner â†’
deathâ†’respawn â†’ loot+insurance), then S14â€“S19 (Force village). **Fold in the 2026-07-02 decisions as you go:**
- S4 `is_lethal_zone` default â†’ `["lawless","contested"]` (+ update its smoke).
...
- Verify: full gate GREEN **+ a two-process headless check** for net slices. On green: **scoped** `git add <paths>`
  ...
  `docs/ASSET_*.md` â€” a parallel session owns the asset pipeline). Commit + log in UNATTENDED_BACKLOG + NIGHTLY_HANDOFF.
- `C:\SW_MUSH` is STRICTLY READ-ONLY. `.uid` files are tracked â€” commit them with their `.gd`.
