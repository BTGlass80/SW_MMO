...
Status as of 2026-06-25: **M1.1 â†’ M1.5 complete and verified, plus M2.0â€“M2.2
(zone/security Director + world events), Waves C (chargen + dual-track CP),
D (combat uses the real sheet + equipped gear), and E (E1â€“E27 persistent-world depth
+ hardening + F1/F2/F3) â€” all COMPLETE.** The single-player slice is now a
...
(see the Wave E entry below + `docs/SESSION_HANDOFF.md` Â§0/Â§5); next depth is owner-gated.
...
1. **Multiplayer foundation first** â€” make it actually networked.
2. **WEG action-window combat** â€” real-time movement, attacks resolved
   server-side in short (~5s) D6 action windows. Server owns RNG/seeds.
3. **Player-driven & persistent world** â€” faction influence + territory players
   permanently reshape; guild sieges as a pillar.
4. **Private/fan IP** â€” Star Wars + WEG D6 used freely, no abstraction tax.
...
combat: client sends an intent â†’ server resolves with its own seed â†’ broadcasts.
...
- `scripts/net/world_state.gd` â€” **pure, socket-free authoritative sim**: player
...
- `scripts/net/network_manager.gd` â€” the `Net` autoload. ENet transport, peer
  join/leave, RPCs (`submit_input` clientâ†’server, `apply_snapshot` serverâ†’clients),
...
- `scripts/net/net_world.gd` + `scenes/net_world.tscn` â€” the networked entry. The
...
- `scripts/tests/net_smoke.gd` â€” headless smoke test of `world_state.gd` (join,
...
- The **full `check_project.ps1` suite is green** (import + 2s runtime launch + 34
  ...
  stale â€” the full gate is the bar.
...
- **M1.1 Net core** â€” DONE.
- **M1.2 Shared Mos Eisley + replicated avatars** â€” DONE (logic). `main.gd`'s
...
- **M1.3 Server-authoritative WEG action-window combat** â€” DONE.
...
- **M1.3b Combat netcode wiring** â€” DONE. `NetworkManager` holds the server
  `CombatArena` (loads `prototype_combatants.json`, registers players on join), a
  `submit_fire_intent` RPC, a ~5s window timer â†’ `resolve_window(server_seed)`, and an
...
- **M1.4 Persistence backbone (JSON)** â€” DONE. Pure `scripts/net/persistence_store.gd`
...
- **M1.5 Player identity / nameplate** â€” DONE. Clients pass `--name`; it flows into
  snapshot nameplates, combat envelopes, and the persisted record.
- **M2.0 Zone & security-state scaffold** â€” DONE. Pure `scripts/net/zone_state.gd`:
...
- **M2.1 Territory-claim scaffold** â€” DONE. Pure `scripts/net/territory_model.gd`: an
  ...
  is deliberately NOT built â€” owner-gated tuning.)
- **M2.2 Director world events** â€” DONE. `zone_state.gd` fires one deterministic event
...
- **Wave C â€” Character creation & dual-track progression** â€” DONE. Pure
  `scripts/rules/chargen_model.gd` (WEG R&E build validation â†’ starting sheet) +
...
- **Wave D â€” Combat uses the character sheet** â€” DONE. `combat_arena.gd` builds each
  ...
  sheet is set.
- **Wave E â€” Persistent-world depth & faithfulness** â€” DONE (E1â€“E27 + hardening +
  ...
  (`force_skills_model`, owner-gated â€” DIV-0011), `security_gate`, `pending_influence_model`,
  ...
  routing, equipment-swap, org claim/release commands with treasury income, the playerâ†’
  ...
  ambient NPC sim. RPC surface grew 8â†’16. Two MEDIUM `register_account` bugs found by
...
- **Status:** the planned multiplayer foundation is feature-complete and hardened; the
  ...
  economy wiring above) is **owner-gated** â€” see `docs/SESSION_HANDOFF.md` Â§5.
