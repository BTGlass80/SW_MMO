# Territory Siege / Hostile-Takeover — Deliberate-Tempo State Machine (Drop 6D+)

Status: DESIGN (docs + data schema only — no gameplay code). Author:
...
`FACTION_TERRITORY_DESIGN.md` §6 (which resolved by an `attacker_fraction`
...
planned, telegraphed** events — not drive-by ganks.
...
| **Effective-security gate** | During an *active assault window only*, the two orgs' members are force-flagged for no-consent PvP at the contested node; the single combat gate honors this | `scripts/net/security_gate.gd` (WORLD_SIM_DESIGN §3.2) |
...
| **Persistence** | The siege row survives a server restart exactly like territory state in `world_state.dat` | `docs/PERSISTENCE_DESIGN.md` §D |
...
different purposes; do not conflate (Guide_11 §2 "Important").

> **Scale reconciliation (flag for the implementer).**
> `FACTION_TERRITORY_DESIGN.md` describes org influence on a 0–150 scale with
> "Foothold = 50". The **live code** (`territory_model.gd`) uses 0–100-ish tiers:
> claim floor **20**, **Dominant ≥ 40**, **Control ≥ 70**. This design anchors to
> the **live code**: the siege declaration gate defaults to **≥ 40 (Dominant)** —
...
> came from the 0–150 doc scale and is **superseded** here. Reconcile the two
...
1. **Target is a live claim** (`territory_claim`) or a **non-citizen-secured city
   expansion node** (city HQ cores are citizen-secured → never siegeable; the
   citadel rule, `FACTION_TERRITORY_DESIGN` §7.3).
...
4. **Attacker org influence in the zone ≥ `attacker_min_influence`** (default 40 /
   Dominant tier).
5. **Declaring character is attacker-org rank ≥ `rank_to_declare`** (default 4 —
   leadership; above the rank-3 claim gate, below the rank-5 city gate).
6. **Attacker treasury holds ≥ `declare_cost_credits`** (default 10 000). On a
   valid declaration the war-chest is **escrowed** out of the treasury into
   `war_chest_credits` on the siege record.
7. **Attacker is under its concurrency cap** —
...
| **Withdraw** | attacker | `rank_to_declare` (4) | → `aborted` (pre-assault) or concede → `resolution`/repelled (during) |
| **Concede / surrender node** | defender | `rank_to_negotiate` (4) | → `resolution` → captured (defender yields to avoid the fight) |
| **Negotiate settlement** | either | `rank_to_negotiate` (4) | data hook only — tribute/ceasefire is an RP/economy detail, not specified here |
| **Commit as ally** | intervening org | `ally_rank_to_commit` (4) | joins a side during `mustering` (§6) |
...
resolution — see §4), and all state transitions.

---

## 3. The phase state machine

Suggested spine, realized: **`declared → mustering → assault(1..N) → resolution →
...
```
      submit_siege_declare (all §2 preconditions pass; war-chest escrowed)
 (none) ───────────────────────────────────────────────▶  DECLARED
                                                             │  grace elapses (declaration_grace_hours, 1h)
   attacker withdraws OR influence < gate                    │  still valid → schedule N assault windows
   during DECLARED  ─────────────▶  ABORTED (full refund)    ▼
                                                          MUSTERING  ◀── allies may commit here only (§6)
                                                             │  warning/prep window (mustering_hours, 24h)
   attacker withdraws (war-chest forfeit)                    │  no forced PvP yet; defender rallies
   OR influence < gate  ────────▶  ABORTED (no refund)       │
                                                             ▼  scheduled assault #1 start reached
                        ┌───────────────────────────────▶  ASSAULT (index i)  ── pvp_consent ACTIVE at scope
                        │                                    │  window = assault_window_hours (2h)
                        │  next assault start reached         │  control meter accrues (§4/§5)
                        │                                    │
                        │        assault window ENDS,        │  early capture: control ≥ capture_threshold
                     LULL ◀──── more assaults remain ────────┤  held for capture_hold_seconds (15m) → RESOLUTION
       (lull_hours, 6h; pvp OFF;   │                          │
        control bleeds toward 0)   │  final assault ENDS  ────┤  OR defender concedes → RESOLUTION
                        │           └──────────────────────────┴────────────────▶  RESOLUTION
   attacker concedes/withdraws ────────────────────────────────────────────────▶ (single-tick tally)
                                                                                     │
                                              control ≥ capture_threshold (75)?      │
                                                   │ yes                    │ no
                                                   ▼                        ▼
                                            outcome=captured          outcome=repelled
                                       (claim → attacker; guard        (defender keeps
                                        dismissed; income reband)       the claim)
                                                   │                        │
                                                   └──────────┬─────────────┘
                                                              ▼
                                                          COOLDOWN  (node locked; cooldown_hours, 168h/7d)
                                                              │  deadline elapses → archive
                                                              ▼
...
|---|---|---|---|---|---|
| `declared` | War declared; **cheap-withdraw grace**; defender formally notified (news + comlink) | `declaration_grace_hours` = 1h | escrow war-chest; snapshot `config`; back-pointer on claim | grace deadline → `mustering`; attacker withdraw / influence < gate → `aborted` (full refund) | off |
| `mustering` | **Warning / preparation** window. Defender rallies; both sides stage; **allies may commit** (§6). No forced PvP | `mustering_hours` = 24h | schedule `assault_count` assault windows at fixed offsets; announce schedule to both orgs | first assault `start_unix` → `assault`; attacker withdraw / influence < gate → `aborted` (war-chest forfeit) | off |
| `assault` | **Assault window** i of N. Forced rival-org no-consent PvP at scope nodes; control meter accrues | `assault_window_hours` = 2h | activate `pvp_consent` for the two orgs at `scope_node_ids` | window end + more remain → `lull`; window end + last → `resolution`; early capture → `resolution`; defender concede → `resolution` | **on** |
| `lull` | **Inter-assault regroup**. PvP off; control **bleeds** toward the defender (0) | `lull_hours` = 6h | deactivate `pvp_consent`; advance `current_assault_index` | next assault `start_unix` → `assault`; attacker concede/withdraw → `resolution` (repelled path) | off |
| `resolution` | **Single-tick tally.** Read control meter; decide capture vs repel; apply effects | 1 tick | read `control_meter.value`; write `outcome.result` | always → `cooldown` | off |
| `cooldown` | **Node lockout.** Outcome recorded; node cannot be re-contested | `cooldown_hours` = 168h (7d) | apply capture/repel effects; set `outcome.lockout_until_unix`; emit news headline | deadline → archive as `captured`/`repelled` | off |
| `captured` | Terminal archive — attacker took the node | — | — | — | off |
| `repelled` | Terminal archive — defender held | — | — | — | off |
| `aborted` | Terminal — declaration cancelled before/at withdrawal; node gets a short `abort_cooldown_hours` (24h) lockout | — | (no capture) refund per phase | — | off |
...
At the `declared → mustering` transition the server computes a fixed schedule of
`assault_count` windows so **both orgs know exactly when the fights are** — the
...
With defaults (1h grace, 24h mustering, 3× 2h assaults, 6h lulls) a full siege
spans **≈ 43 h wall-clock** from declaration to resolution, of which **only 6 h is
forced-PvP contest** (3 × 2 h), all at pre-announced times, followed by a **7-day
...
prototype population can compress mustering/lull dramatically — see §8.)*
...
> 72 h, ≥ `lull_hours` apart) so both orgs pick a mutually-live slot. Recommend
...
no RNG in siege resolution** — control moves only from *already-resolved* combat
...
- **During an `assault` window**, resolved scoring events (§5) add/subtract from
...
1. **Early capture (hold-for-duration).** During any `assault` window, `value`
   stays **≥ `capture_threshold`** (default **75**) **continuously for
   ...
   cleared the instant `value` drops below it; when `now - hold_since_unix ≥
...
2. **Final tally.** If no early capture fires, at `resolution` (immediately after
   the **final** assault window ends) the server reads `value`. **`value ≥
   capture_threshold` ⇒ captured; otherwise ⇒ repelled.**

A threshold of **75** (not 50) means the attacker must **clearly dominate**, not
merely draw — the defender holds on anything short of decisive control. This is
...
future CP reward (§13). The **`control_meter`** is the separate, clamped, bleeding
...
## 5. Scoring events → control deltas
...
consumes the resolved envelope. All deltas are `config`-tunable (§8). During an
`assault` window only:

| `kind` | Side | `control_delta` (default) | Audit `points` | Notes |
|---|---|---|---|---|
| `pvp_kill` | killer's side | ±`control_per_pvp_kill` = 8 | 6 | Attacker kill +8, defender kill −8. **Depends on PvP unlock — see §13.** |
| `guard_defeated` | attacker | +`control_per_guard_defeated` = 12 | 10 | Breaking the defender's stationed guard NPC (PvE — works today) |
| `control_hold_tick` | side with more living in-scope members this slow tick | ±`control_per_hold_tick` = 2 | 2 | Holding ground over time; deterministic from server presence count |
| `objective` | either | ±`control_per_objective` = 15 | 12 | Optional node objective (raise a banner, hold a console) |
...
player PvP remains owner-gated (§13).
...
may join a side, but **only by committing during the `mustering` warning window** —
...
- An intervenor needs **`ally_min_influence` = 20** (claim-floor presence) in the
  zone — it must have some standing there, not teleport in from nowhere.
...
- Allies **cannot declare, withdraw, concede, or negotiate** — those remain the
...
- On capture, the node transfers to the **attacker principal only** (allies gain
  reputation/relationship, not the claim — spoils-sharing is an economy/RP hook,
  not specified here).

**Tunable alternatives** (`config.intervention_mode`): `none` (strictly the two
principals — the conservative v1 fallback if the owner wants the simplest thing)
or `open` (allies may commit any time before the final assault — more chaotic,
...
- **Node cooldown.** `cooldown_hours` = 168 h (7 days) after any resolved siege
  ...
  declare/abort).
- **Per-org attack cap.** `max_concurrent_attacks_per_org` = 1 — an org runs at
  most one outgoing siege at a time. (Defenses are unbounded; being attacked is
  not consensual.)
- **Influence gate.** The attacker must have earned **Dominant** influence (≥ 40)
  in the zone — no drive-by sieges.
- **Economic weight.** A 10 000 cr war-chest is escrowed and **forfeit** if the
  ...
  during the siege — a besieger who over-extends can lose ground elsewhere.
...
- **Citadel immunity.** Citizen-secured city cores can never be sieged
  (`FACTION_TERRITORY_DESIGN` §7.3); only contestable outer/expansion nodes can.

---

## 8. DEFAULTS — the tunable-constants table (deliberate tempo)
...
owner rulings. The pure model (§12) should expose each as a named constant with
...
| `capture_hold_seconds` | 900 (15 min) | 60 | continuous time ≥ threshold in an assault for early capture |
...
| `attacker_min_treasury` | 10000 | 10000 | attacker treasury must hold ≥ this to declare (= cost) |
...
`PERSISTENCE_DESIGN.md` §D, or the JSON blob in `world_state.dat` for the
...
| `pvp_consent` (active + scope) | **No — re-derived** | recomputed from `state` on the first slow tick (only true while `state == assault`); never trusted from disk |
...
during downtime (declared→mustering→assault→lull→…→resolution), applying bleed for
non-assault spans. Because resolution has no RNG, a siege that *should* have
resolved during downtime resolves identically on catch-up — matching
`PERSISTENCE_DESIGN.md` §"Siege resume specifically". Transient PvP/consent flags
...
- **Effective-security gate.** During `state == assault`, the combat-initiation
  ...
  member as **permitted no-consent PvP**, regardless of the zone tier — the one
  documented exception to consent (WORLD_SIM_DESIGN §3.3). Outside that
  ...
  consumer and lands **with** the PvP unlock (§13).
...
- **Territory model.** On capture, call the claim-transfer path (set
  `territory_claim.org_id` ← attacker, dismiss guard, re-evaluate
...
- **News feed.** Every terminal outcome emits a Director news headline
  (WORLD_SIM_DESIGN §2.4 / `director_log`), the "did you see what happened"
  social currency.

---

## 11. Divergence note — DIV-0020 (add to the ledger BEFORE implementation)
...
> §7 specced a "contest state machine, 7-day timer, rival-org no-consent PvP,
> hostile takeover" but never delivered it ("Planned") | Deliberate-tempo
> multi-phase machine (declared→mustering→assault(1..N)→resolution→cooldown) on a
...
## 12. Implementation slices (ordered — for the main dev loop)
...
1. **`[PAR]` Pure `scripts/rules/siege_state_model.gd` + smoke.** A RefCounted,
   ...
   wall-clock `now` (declared→mustering→assault→lull→resolution→cooldown→archive,
   ...
   ally commit — wired into `tools/check_project.ps1`. **Depends on nothing new;
   build first.** *(This is engineer work — the pure-model + test slice — not part
...
2. **`[HOT]` `network_manager` declare/join/status RPCs + Director-tick hook.**
   ...
   `network_manager.gd`, owned by another session — sequence it after slice 1 and
   coordinate; described here as the wiring contract, not implemented in this
   design task.)*

---

## 13. Open owner decisions & dependencies (flagged — NOT decided here)
...
2. **Death / loot penalty in the assault window (DIV-0006, decided 2026-07-01).**
   ...
   siege inherits it — no separate ruling needed, but the assault-window death
...
3. **CP rewards for siege participation (CP-pace decision).** The schema logs
   ...
   owner decision (WORLD_SIM_DESIGN §7.3).
4. **Assault start-time authorship** (§3.3): auto-schedule vs attacker-proposed.
   Recommend auto for v1.
5. **Intervention default** (§6): `mustering_only` + 1-per-side recommended;
   `none` is the conservative fallback.
6. **Deliberate vs prototype timer profile** (§8): the 24 h/7 d deliberate profile
...
- `data/schemas/siege_state.schema.json` — **the finalized siege record** (this
  design's data contract).
- `data/schemas/territory_claim.schema.json` — the contested target; `siege`
  back-pointer enum needs the §9 follow-up update.
- `data/schemas/security_zone.schema.json` — the base tier a siege respects.
- `data/schemas/faction_zone_state.schema.json` — Director zone posture (separate
