...
This document specifies the **player-driven territorial layer** â€” the part of the
...
Â§7: *"Contest state machine, 7-day timer, rival org no-consent PvP, hostile
takeover â€” Planned"*).
...
`security_zones_design_v1.md` â€” adapted from MUSH-staff-driven commands into an
...
> tables with different purposes, bridged only by faction axis for narrative â€”
> exactly as the MUSH keeps them apart (Guide_11 Â§2 "Important"). Do not conflate.
...
Each org tracks an **influence score per zone**, 0â€“150 (Guide_11 Â§2). It is the
...
**Earning** (Guide_11 Â§2; starting values, owner-tunable): member present in zone
...
**Decay:** if no org member is present in a zone for 48 h, influence decays âˆ’5/day;
active presence resets the timer. **You must maintain a real presence** â€” you
cannot invest once and walk away. Sustained absence ultimately lapses claims.

---

## 3. Claiming nodes (Drops 6Aâ€“6C, already proven in the MUSH)

Schema: `data/schemas/territory_claim.schema.json`. Authoritative per-node state.

**Claim rules** (Guide_11 Â§3): rank 3+; standing in the node; node is in a
**contested or lawless** zone (secured cannot be claimed); â‰¤3 claims/zone, â‰¤10/org;
...
**What a claim grants:**
- **Security upgrade for owning members** â€” a lawless node is treated as
  *contested* for the org (PvP-consent protection on home turf). This is step 3 of
  the effective-security function (WORLD_SIM_DESIGN Â§3.2).
- **A visible claim tag** in the zone readout.
- **A guard NPC slot** (Â§4).
- **Passive resource income** (Â§5).

**Guard NPCs** (Â§4) and **resource nodes** (Â§5) follow; **maintenance lapse**: a
...
Rank 3+ may station one aggressive guard NPC per claimed node (Guide_11 Â§4):
...
siege (defeating the guard moves the contest score â€” Â§6.5). Guards resolve combat
...
schedule, WORLD_SIM_DESIGN Â§4), scaled by `(security, influence_tier)` â€”
**lawless yields more than contested** (Guide_11 Â§5; higher risk, higher reward):

| Security | Tier | Daily yield (starting bands) |
|---|---|---|
| Contested | Foothold | 50â€“150 cr |
| Contested | Dominant | 100â€“300 cr + 1â€“2 metal |
| Contested | Control | 150â€“400 cr + 1â€“2 metal + 1 rare |
| Lawless | Foothold | 75â€“200 cr |
| Lawless | Dominant | 150â€“400 cr + 2â€“4 metal + 1â€“2 chemical |
| Lawless | Control | 250â€“600 cr + 2â€“4 metal + 2â€“4 chemical + 1â€“2 rare |
...
## 6. The siege / hostile-takeover state machine â€” **Drop 6D (headline PvP loop)**
...
safe base (its city/claims) and try to take a rival's turf â€” and must defend its
own.
...
- Target node is a **territory claim** (or a city expansion node â€” Â§7.4) in a
  **contested or lawless** zone. **Secured zones and city-upgraded-to-secured
  citizen rooms can never be sieged** (the citadel rule, Â§7.3).
- Attacker org has **â‰¥ Foothold (50) influence** in the zone
...
```
   ...
   (none) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â–¶  DECLARED
                                                  â”‚  warning_hours elapse (default 24h)
                                                  â”‚  defender is warned; window telegraphed
                                                  â–¼
                                               ACTIVE  â—€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”
   attacker influence falls below gate            â”‚                   â”‚ (contest continues
   during DECLARED  â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â–¶ ABORTED        â”‚                   â”‚  through the window)
   (terminal; no transfer)                        â”‚  contest_window_hours elapse (default 48h)
                                                   â”‚  OR attacker concedes / defender wipes attacker
                                                   â–¼
                                               RESOLVING
                                       (server tallies score this tick)
                                                   â”‚
                         attacker_fraction â‰¥ control_threshold (default 0.6)?
                              â”‚ yes                          â”‚ no
                              â–¼                              â–¼
                          CAPTURED                        REPELLED
                  (claim â†’ attacker;              (defender keeps claim;
                   income/guard reset)             both into lockout)
                              â”‚                              â”‚
                              â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¬â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜
                                             â–¼
                                          LOCKOUT  â”€â”€â”€â”€ lockout_hours elapse (default 168h / 7d) â”€â”€â”€â–¶  (siege archived)
```

**State meanings** (`siege_state::state` enum):

- **`declared`** â€” the **defender-warning / telegraph** window. The contest is
  ...
  territorial moves are *visible* and play out over real time (Guide_12 Â§3's 24h
  city-expansion telegraph; rivals "know where you're pushing"). Default
  `warning_hours = 24`.
- **`active`** â€” the **contest window**: forced rival-org no-consent PvP is live at
  the node + declared adjacency (Â§6.4); scoring accrues (Â§6.5). Default
  `contest_window_hours = 48`. The MUSH's "7-day timer" is captured by the full
  declaredâ†’activeâ†’lockout span being multi-day and owner-tunable â€” concretized
  into named phases rather than one opaque countdown.
- **`resolving`** â€” a single-tick tally state: the server reads `score`, computes
...
- **`lockout`** â€” post-resolution cooldown; the node cannot be re-contested until
  `lockout_hours` elapse (default 168 h / 7 days). Prevents perpetual griefing of
  one node.
- **`captured`** (terminal) â€” claim ownership transfers to the attacker; guard
  dismissed, income band re-evaluated for the new owner, news headline emitted.
- **`repelled`** (terminal) â€” defender keeps the claim; news emitted.
- **`aborted`** (terminal) â€” declaration cancelled before `active` (e.g. attacker
...
`pvp_consent.scope_node_ids` (the claim node + declared adjacency) â€” **regardless
...
security-zone consent (WORLD_SIM_DESIGN Â§3.3) applies. The effective-security
function (WORLD_SIM_DESIGN Â§3.2) consults the siege state to apply this; a
bystander third org is unaffected unless it joins (owner question Â§9).
...
`resolving`, `attacker_fraction â‰¥ control_threshold` (default 0.6) â‡’ capture. A
threshold above 0.5 means the attacker must *clearly* win, not merely tie â€” the
defender holds on a draw.

### 6.6 Outcome effects

- **Captured:** `territory_claim.org_id` â† attacker; guard dismissed; income band
...
(WORLD_SIM_DESIGN Â§2.4) â€” the "did you see what happened last night?" social
...
- **Influence gate:** an attacker must have *earned* a foothold to declare â€” no
...
- **City citadel immunity:** citizen-secured city rooms can't be sieged (Â§7.3).
- **Maintenance still applies:** a besieging attacker who lets their own claims
  lapse loses ground elsewhere â€” sieges have an opportunity cost.

---

## 7. Player cities (Guide_12)

Cities are the largest persistent player-owned structures â€” a *named place* an org
grows into. They sit atop territory and amplify it.

### 7.1 Founding & expansion (rank-5 org leader)

Prereqs (Guide_12 Â§2): rank-5 leader; a tier-5 HQ in a **contested or lawless**
zone (cities cannot be founded in secured zones â€” frontier/underworld governance
by design); **â‰¥ 50 influence** in the zone; treasury holds the founding cost
(by HQ subtype â€” starting values 25k / 75k / 200k cr). Expansion claims adjacent
...
act* rivals can watch â€” the same telegraph philosophy as the siege warning window.
...
### 7.3 The citizen security upgrade â€” the **citadel rule**

Inside a city room **as a citizen**, effective security is upgraded one step:
contested â†’ **secured**, lawless â†’ **contested** (Guide_12 Â§6). This is step 2 of
the effective-security function (WORLD_SIM_DESIGN Â§3.2) and is the **most-permissive
last word for citizens** â€” even a hostile Director downgrade can't make a citizen
less safe than their own city allows. Outsiders get the zone base. **A
citizen-secured room cannot be sieged** (Â§6.2) â€” the city is a citadel a small
...
A city's **HQ City-Center rooms are permanent and citizen-secured â†’ never
...
frontier â€” the right strategic shape.

### 7.5 City economy & governance (data hooks)

Tax (0â€“10%, Mayor-set under a Founder rate cap) skims city-room commerce into the
treasury, invisible to the payer (Guide_12 Â§5). `+city home` gives citizens an
...
All on the **slow-tick schedule** (WORLD_SIM_DESIGN Â§4), never the 20 Hz path:
...
## 9. OPEN OWNER DECISIONS (flagged â€” NOT decided here)
...
1. **Death / loot penalty in siege/lawless PvP.** The whole siege loop assumes
   *some* stakes for dying in a forced-PvP window, but the exact model
   ...
   decision (WORLD_SIM_DESIGN Â§7.2). Recommendation there: security-graded, lawless
   harsh â€” which gives sieges weight without a separate ruling here.
2. **Force / Jedi in sieges.** Whether Force-users get any siege advantage depends
   on the Force-scarcity/access decision (WORLD_SIM_DESIGN Â§7.1). The siege machine
...
3. **CP rewards for siege participation.** Whether winning a siege grants CP, and
   how much, couples to the CP-pace decision (WORLD_SIM_DESIGN Â§7.3). The siege
...
3. **Capture threshold.** `control_threshold` default 0.6 (attacker must clearly
   win). Higher (defender-favored) or 0.5 (coin-flip on a draw)? Recommend â‰¥ 0.55
...
6. **City expansion-room siegeability.** Confirm Â§7.4: non-citizen-secured city
   expansion rooms are siegeable while the HQ core is immune. Recommend yes â€” it's
...
- `data/schemas/territory_claim.schema.json` â€” one claimed node.
- `data/schemas/siege_state.schema.json` â€” one Drop-6D contest (the state machine).
- `data/schemas/security_zone.schema.json` â€” the security gradient sieges respect.
- `data/schemas/player_persistence.schema.json` â€” org membership + city role.
