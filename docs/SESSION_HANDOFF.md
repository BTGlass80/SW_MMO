# SESSION HANDOFF â€” start here in a clean session
...
Read this top to bottom, then execute **Â§1 First actions**. You should not need to ask
the owner anything to begin â€” the work is queued and the guardrails are explicit.
...
`docs/WAVE_G_BACKLOG.md` Â§ Delta follow-ups (G14â€“G18 + the PT1 prep track), and the gate
PRINTS the wired smoke + RPC counts â€” trust the gate output and the git log over any count
...
- **Built & verified:** M1.1â€“M1.5 net core/persistence/nameplates â†’ M2.0â€“M2.2 zone/
  security Director + world events â†’ **Wave C** (chargen + dual-track CP) â†’ **Wave D**
  (combat uses your sheet + equipped gear) â†’ **Wave E (E1â€“E27) COMPLETE** â€” the full
  ...
  test guards; then the [HOT] netcode â€” multiple zones with per-player snapshot routing,
  equipment-swap, org claim/release commands with treasury income, the playerâ†’influence
  ...
  long **follow-up series F1â€“F65** (each gate-green + two-process verified) that grew the slice
  into a genuinely playable MMO and then hardened it. Highlights:
  - **Wound/medical loop (F7â€“F10, F17, F19):** natural self-recovery (DIV-0012) + First Aid by a
  ...
  - **Multi-zone world (F11â€“F14):** in-session zone TRAVEL (DIV-0014, persisted) + zone-scoped
  ...
  - **Combat correctness (F18):** equipped-weapon SKILL drives the attack pool + melee `STR+ND`
    damage (wires derived-stats melee) â€” latent fix, melee is acquirable only once an economy exists.
  - **Social (F2, F20, F22, F25â€“F27):** zone say/emote + global ooc + ORG chat (cross-zone) + a **GUI chat
  ...
  - **Correctness-audit pass (F28â€“F32):** reset the First-Aid retry gate on full heal (F28); org
    own-vs-total claim count (F29); refresh the sheet on CP award (F30); **the WEG "out" state** â€” an
    ...
    faithful-WEG but currently **LATENT** (nothing damages a player in live play yet â€” the wound loop
    becomes reachable only when the owner-gated PvP / hostile-NPC / death path lands).
  - **Robustness/guards (F4â€“F6, F10, F14, F21):** record-cache eviction on disconnect; FIVE [HOT]
    composition guards (claims/auth/First Aid/zone/chat) locked into the gate.
  - **Combat-depth + medical arc (F33â€“F54):** the owner-directed **non-lethal medical loop** (F44,
    DIV-0016 â€” the B1 remote returns real fire capped at Wounded(2), so the woundâ†’recoveryâ†’First-Aid
    loop is reachable end-to-end) + a full WEG tactical layer â€” two-way return fire (F45), Perception
    initiative (F48), cover / active-dodge / full-dodge stances (F50â€“F52), and target/condition HUDs
    (F46/F47); plus faction legibility (rank authority + allegiance + online org count, F34â€“F37/F53)
    and chat/`/help`+keybinds UX (F39â€“F42/F54).
  - **WEG-fidelity + durability + hardening pass (F55â€“F65):** the **Force-Point DAMAGE-doubling fix**
    (F55 â€” FP doubled attack/dodge/soak but not damage) and the **armor pip-only "+2" parse fixes**
    (F60 + F64 â€” TWO parse sites: `d6_rules.parse_pool` read "+2" as 2D, and `armor_condition_model`
    then dropped a "+2" armor entirely; both now treat a no-"D" token as +N pips, restoring WEG soak);
    the **full restart-durable persistent world** â€” atomic character saves + crash recovery (F56),
    and a single `world_state.dat` carrying faction-influence/Director/pending (F58), org claims +
    treasuries (F59), and claim-gating territory-influence (F61); **auth hardening** â€” an
    un-registered peer is barred from entering the world (F57) AND from chat (F62); earn-loop gate
    coverage (F63, killsâ†’claim threshold); and **combat-envelope zone-scoping** (F65 â€” combat was
    broadcast galaxy-wide; now same-zone only, like chat). All gate-green + two-process verified.
- **Green bar (current truth):** the **full** `tools/check_project.ps1` passes â€” **59
  GDScript smokes** + 7 python + import + launch (green at every commit). DIV-0001..0016.
- **STATUS: Wave E + the F1â€“F65 follow-ups are DONE; the prototype is a playable, populated,
  traversable multi-zone MMO** (chargen/progression â†’ species-paced movement â†’ combat (CP/FP/cover/
  dodge/initiative) â†’ full visible non-lethal medical loop â†’ equip â†’ travel/presence â†’ org
  claims+treasury, restart-durable â†’ say/ooc/org chat + command bar w/ GUI input â†’ character-sheet
  panel â†’ news â†’ rendered NPCs), all gate-guarded. **The unblocked, non-owner-gated backlog is DRY
  and the frontier has been EXHAUSTIVELY audited** â€” beyond the F33â€“F54 depth arc, the F55â€“F65 pass
  ...
  space/presentation/core audit + direct RPC/RNG/data/broadcast audits** â€” every angle now clean or
  fixed. The substantive remainders cluster behind the **economy/vendor SPEND side** (unlocks melee
  + the orphaned creature_spawn/vendor/reputation models) and the parked owner forks (Â§5: siege,
  ...
  `UNATTENDED_BACKLOG.md` Log â€” F1â€“F65.)
...
1. **Orient:** read `CLAUDE.md`, this file, and `docs/UNATTENDED_BACKLOG.md` (the Wave E
   queue + Guardrails + the F1â€“F65 Log). Skim `docs/DIVERGENCE_LEDGER.md` (DIV-0001..0016).
2. **Confirm the baseline is green** before changing anything:
   ...
   ```
   and `git log --oneline -5`. If RED on arrival, fix or report â€” do not build on red.
3. **Arm the loop** so it keeps going while you work. The previous driver cron was
   ...
   **CronCreate** with the contract in **Â§2** (cron `6,16,26,36,46,56 * * * *` â‰ˆ every
   10 min, idle-only, `recurring:true`, `durable:false`). Recurring crons auto-expire
   after 7 days â€” re-arm if this runs that long.
4. **Check the queue.** As of 2026-07-03 the ACTIVE queue is **`docs/WAVE_G_BACKLOG.md`
   Â§ Delta follow-ups** (G14â€“G18, then the owner-approved PT1 prep track: G8 auth bundle,
   server watchdog, 20-bot soak, envelope replay). Take the top unblocked batch (the Â§3
...
5. **Keep going while there is unblocked, non-owner-gated work; otherwise HOLD.** Don't ask
   ...
   steer â€” do not fabricate owner-level decisions or churn on marginal additions. Park the
   Â§5 owner-gated forks BLOCKED; report a real, persistent blocker.

---

## 2. The loop contract (paste this as the CronCreate `prompt`)

> Unattended SW_MMO development tick â€” single driver for gameplay/netcode/rules/world-
> sim/tests/docs. Read `docs/SESSION_HANDOFF.md` (parallelization playbook + guardrails)
> and `docs/WAVE_G_BACKLOG.md` Â§ Delta follow-ups (the active queue) before acting. Take
> the top unblocked, non-owner-gated item(s). Smokes assert what SHIPS â€” a deliberate
> behavior change reddening an old smoke means UPDATE THE ASSERTION, never revert the fix.
> Push `origin master` after each gate-green commit. **Parallelize when it makes sense:** batch 2â€“4 `[PAR]` pure-
...
> `tools/fetch_assets.py`, `tools/asset_sources.json`, `docs/ASSET_*.md` â€” a parallel
...
> stop â€” keep developing. SKIP/park only genuinely owner-gated forks (siege/Drop-6D
...
> items A1b/P1). Do NOT create new cron jobs and do NOT call ScheduleWakeup â€” THIS cron is
...
**(a) Within a slice (always):** use the **Workflow** tool to raise quality â€”
...
- **`[PAR]` slices** (pure model + test, or docs/test-only) each create their **own new
  ...
  `docs/DIVERGENCE_LEDGER.md` â€” small, append-only edits.
- **Recommended per-tick shape:** spin up a Workflow that implements **2â€“4 `[PAR]`
...
- **`[HOT]` slices** edit `network_manager.gd` and/or `net_world.gd`. Do them **one at a
  time on the main tree** â€” never two `[HOT]` slices in parallel, and don't run a `[HOT]`
...
**Order of attack:** clear the `[PAR]` pure substrate first (E1â€“E20 ish) â€” it adds test
coverage and builds the pure models the `[HOT]` features consume (E8â†’E24, E9â†’E23,
E21â†’E23/E27) â€” then land the `[HOT]` features one per tick.
...
- **Asset pipeline is owned by a PARALLEL session.** NEVER stage, revert, or edit
  ...
  **only** your own changed files: `git add <paths>` â€” **never** `git add -A`. A RED slice
  reverts **only its own** files (`git checkout -- <your paths>`) â€” never a blanket
  `checkout -- .` / `reset --hard` / `clean` (that would destroy the asset session's work).
  The Log shows the asset session also appends entries â€” leave their lines alone; only add
  your own.
...
- **Clone Wars era only.** GCW/Imperial/Rebel/stormtrooper framing is a bug unless
  deliberately recast. `content_smoke` enforces a forbidden-GCW-ship list â€” keep it green.
...
## 5. Owner-gated â€” PARK, never auto-build

If a slice would require deciding any of these, stop it and mark it
`BLOCKED: needs owner decision â€” <which>` in the backlog. Do **not** pick a number and proceed.

> **UPDATE 2026-07-02 (owner, via a parallel design session) â€” four parked forks below are now RESOLVED:**
> - **Siege â†’ DESIGN APPROVED, deliberate tempo.** Full state-machine design + finalized schema shipped this session:
>   **`docs/SIEGE_DESIGN.md`** + **`data/schemas/siege_state.schema.json`** (`declaredâ†’musteringâ†’assaultÃ—3â†’resolutionâ†’
>   cooldown`; ~43 h wall-clock, control-meter capture at â‰¥75 held 15 min, per-org concurrent cap 1, 7-day node
>   cooldown â€” all numbers are tunable DEFAULTS). Wiring is queued for the main loop (**add DIV-0020 before coding**).
>   Only the siege *numbers* stay owner-tunable; the design itself is no longer gated.
> - **PvP consent â†’ DECIDED: all three layers** â€” zone-based open-PvP in lawless **+** opt-in duels (work in safe zones,
>   default non-lethal) **+** bounty-as-consent. Unified design shipped: **`docs/PVP_CONSENT_DESIGN.md`** (deterministic
>   resolver + zoneÃ—duelÃ—bounty truth table). NOTE: the dev loop already shipped **Layer 1 (zone-based)** as
...
> - **CP award-RATE â†’ DECIDED: MEDIUM / keep current pace.** No longer gated; no code change needed.
> - **LLM Director â†’ DECIDED: DEFERRED** for the prototype â€” deterministic Director only, **no LLM/API in the live
>   tick**; revisit post-slice.
>
> **Still genuinely gated (unchanged):** the owner-gated *visual* checks â€” A1b building-model swaps + scale tuning, P1
> client polish, and the open two-client GUI visual check â€” they need a human at the GUI. The bullets below are kept
...
- **Siege / Drop-6D** phase durations, capture threshold, third-party intervention,
  per-org siege cap (the `siege_state.schema.json` state machine â€” design only).
...
- **LLM "Director flavor"** layer at launch (deterministic baseline already ships; an LLM
  in the tick is owner-gated â€” do not wire an API in).
...
- **Refactoring the hot autoload** (`network_manager.gd`) into extracted testable helpers
  is an architecture call â€” leave it unless the owner asks.

Decided & usable (NOT parked): **DIV-0006** death-penalty shape, **DIV-0007** dual-track CP.

### 5a. What each gate UNLOCKS (already built + tested â€” ROI map for the owner)
...
**reachable in live play**. Highest leverage first. (This is a prioritization aid â€” the loop
does NOT pre-decide any of these.)

- **Player-damage path** â€” âœ… **PARTIALLY UNLOCKED (F44, owner-directed 2026-06-25).** The
  NON-LETHAL portion is now LIVE: the B1 training remote returns real fire capped at Wounded(2)
  (DIV-0016), so the whole wound/medical loop is reachable and verified end-to-end â€” `wound_ladder`
  ...
  recovers (self or medic). STILL GATED: the **LETHAL** path â€” real death (incapacitated/mortally/
...
- **Economy / vendor** (needs: prices, spawn rates, item values) â€” unlocks three modeled-and-
...
- **Force / Jedi ACCESS policy** (rarity, how one becomes force-sensitive) â€” unlocks
  `force_skills_model` (E6 â€” the off-by-default data hook is already built; only the access
  economy is gated).
- **Siege durations / capture threshold** â€” unlocks the `siege_state` schema state machine on top
  of the live territory-claim substrate (claims + treasury income + rank authority: E9/E23/F29/F34,
  validated F38).
- **CP award-RATE tuning** (DIV-0007 dual-track mechanic already live) â€” only the numbers gate how
  fast progression/prestige accrue; the earning + spending paths work (C4/F30).
- **LLM Director flavor / visual A1b+P1** â€” additive polish on a working deterministic baseline.
...
- **Full gate:** `.\tools\check_project.ps1 -GodotConsole "C:\Godot 4\Godot_v4.6.3-stable_win64_console.exe"`.
  It runs python unittests â†’ import â†’ 2s launch â†’ all GDScript smokes, and FAILS on any
...
- **Two-process headless check (net slices):** start the server in the background
  ...
  poll its log for `server listening` (use `ping -n 2 127.0.0.1` to wait â€” `sleep` is
  ...
  --raise-skill` etc. as needed â€” there is NO `--quit-after` client flag; the client runs
  ...
  `SCRIPT ERROR|is_inside_tree` â€” the runtime-load check catches what the gate can't.
...
- **Pure rules** `scripts/rules/*`: `d6_rules` (pools/Wild Die/scale/soak/wound chart),
  ...
  smoked but **orphaned** â€” wiring needs the owner-gated economy).
- **Net layer** `scripts/net/*`: `world_state` (pure 20 Hz movement truth),
  `combat_arena` (pure ~5s WEG action-window resolution, server seed),
  `persistence_store` (atomic `.tmp`â†’rename JSON per character + crash-recovery, F56; PLUS a single
  server-global `world_state.dat` carrying the restart-durable Director/territory state â€” faction
...
- **RPC surface (21, all in `network_manager.gd`):** clientâ†’server (`any_peer`, 10)
  ...
  `submit_heal` (F8 First Aid), `submit_change_zone` (F11 travel); serverâ†’client
...
- **Tick model:** 20 Hz movement (per-player speed from species, DIV-0015) + snapshot; ~5s
  combat window (`--combat-window`); ~30s Director tick (`--director-tick`) â€” folds influence,
...
- **Data** `data/*.json`: species(9)/skills(76)/weapons(32)/armor(23) are consumed at
  ...
  **modeled, not yet wired into `net_world`** (no live spawns/shops/rep-on-action â€” that
...
- **Design canon** `docs/`: `MULTIPLAYER_FOUNDATION`, `WORLD_SIM_DESIGN`,
  `FACTION_TERRITORY_DESIGN`, `PERSISTENCE_DESIGN`, `DIVERGENCE_LEDGER` â€” reconciled to the
  shipped state by **E1** (no longer stale; see Â§8).

---

## 8. Roadmap docs â€” reconciled (E1, done)

E1 reconciled the roadmap/reference docs to the shipped state, so they are CURRENT â€” do not
re-reconcile them:
- `MULTIPLAYER_FOUNDATION.md` â€” M1.1â€“M1.5 + M2.0â€“M2.2 + Waves C/D/E COMPLETE, with a
  backlog-dry / owner-gated-next status.
- `NIGHTLY_HANDOFF.md` â€” the full F1â€“F65 narrative; the old self-contradicted "loop STOPPED
  at Wave C" line was removed.
- `NEXT_DECISIONS.md` â€” archived early artifact (long resolved).
- `UNATTENDED_LOOP.md` + the backlog Guardrails â€” the stale "`--import` can fail" caveat was
  corrected (A0's colormap fix is live; the full `check_project.ps1` is the bar).

For the LIVE state trust Â§0 above + the `UNATTENDED_BACKLOG.md` Log (newest entries first).
