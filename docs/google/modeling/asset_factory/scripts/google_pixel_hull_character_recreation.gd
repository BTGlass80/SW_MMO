...
const OUT_ROOT := "res://docs/google/modeling/asset_factory/generated/google_pixel_hull_character_recreation_v0"
...
var _source_paths := {
	"front": SOURCE_DIR + "/commander_front_card_16x28.png",
	"side": SOURCE_DIR + "/commander_side_card_10x28.png",
}
...
func _run() -> void:
	...
	_write_manifest()
	_write_review()
	print("Google pixel-hull character recreation generated %s captures" % _captures.size())
	quit()
...
func _draw_front_card(image: Image) -> void:
	...
	var orange := Color("#f17829") # Google recreation: 212th Orange
	...
	_fill_rect(image, 7, 1, 2, 3, orange) # Orange helmet stripe
	
	# Rangefinder (stalk + head)
	image.set_pixel(4, 2, dark) # Stalk
	image.set_pixel(3, 1, dark) # Head
	image.set_pixel(3, 2, dark) # Head

	# Torso & arms
	_fill_rect(image, 6, 7, 4, 7, armor)
	_fill_rect(image, 5, 8, 6, 5, armor_shadow)
	_fill_rect(image, 7, 8, 2, 6, orange) # Orange chest stripe
...
func _draw_side_card(image: Image) -> void:
	...
	_fill_rect(image, 4, 1, 1, 3, orange) # Orange stripe
	
	# Rangefinder
	image.set_pixel(4, 1, dark)
	image.set_pixel(4, 2, dark)
...
func _build_source_card_scene() -> Node3D:
	var root := _base_scene("GoogleRecreationSourceCards")
...
func _build_flat_vs_volume_scene() -> Node3D:
	var root := _base_scene("GoogleRecreationFlatVsVolume")
	var front := Image.load_from_file(ProjectSettings.globalize_path(_source_paths["front"]))
	var side := Image.load_from_file(ProjectSettings.globalize_path(_source_paths["side"]))
...
func _build_three_quarter_scene() -> Node3D:
	var root := _base_scene("GoogleRecreationThreeQuarter")
	var front := Image.load_from_file(ProjectSettings.globalize_path(_source_paths["front"]))
	var side := Image.load_from_file(ProjectSettings.globalize_path(_source_paths["side"]))
...
func _build_rotation_sheet_scene() -> Node3D:
	var root := _base_scene("GoogleRecreationRotationContactSheet")
	var front := Image.load_from_file(ProjectSettings.globalize_path(_source_paths["front"]))
	var side := Image.load_from_file(ProjectSettings.globalize_path(_source_paths["side"]))
...
func _material(color: Color) -> StandardMaterial3D:
	...
	mat.roughness = 0.9
	# Emmisive lights if orange is chosen (let's keep standard orange flat, but cyan lights still glow if needed)
...
func _write_manifest() -> void:
	...
	var file := FileAccess.open(OUT_ROOT + "/pixel_hull_recreation_manifest.json", FileAccess.WRITE)
...
func _write_review() -> void:
	...
	lines.append("# Google Pixel Hull Character Recreation v0")
	lines.append("")
	lines.append("Generated: %s" % Time.get_datetime_string_from_system(false, true))
	lines.append("Generator: `docs/google/modeling/asset_factory/scripts/google_pixel_hull_character_recreation.gd`")
	lines.append("")
	lines.append("## Purpose")
	lines.append("")
	lines.append("Google's recreation of the Clone Trooper model using Codex's pixel-hull generator strategy.")
	lines.append("We transitioned the team markings to orange (212th Attack Battalion style) and added a helmet rangefinder accessory.")
	lines.append("")
	lines.append("## Source Cards")
	lines.append("")
	lines.append("- `source_images/commander_front_card_16x28.png`")
	lines.append("- `source_images/commander_side_card_10x28.png`")
	lines.append("")
	lines.append("![front card](source_images/commander_front_card_16x28.png)")
	lines.append("")
	lines.append("![side card](source_images/commander_side_card_10x28.png)")
	lines.append("")
	lines.append("## Stats")
	lines.append("")
	...
	for entry in _captures:
		...
		lines.append("")
	lines.append("## Comparison with Codex Trooper")
	lines.append("")
	lines.append("The Google rangefinder commander recreation successfully maps the orange command markings and rangefinder accessory using the same visual hull intersection mechanics.")
	lines.append("This shows the flexibility of the pixel-hull method for rapidly swapping colors, textures, and geometric accessories (like rangefinders, pauldrons, or antennas) without manual mesh modification.")
	lines.append("")
