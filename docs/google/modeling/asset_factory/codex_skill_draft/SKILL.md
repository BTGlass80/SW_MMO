...
- Terrain/buildings/Cantina: descriptions and YAML topology -> semantic floor/detail pixel cards -> Godot room/building kit -> collision/walk masks/sockets where relevant -> Blockbench identity modules only where needed -> Godot capture.
- Room kits/Cantina LOD: room graph or layout -> semantic pixel floorplan card plus detail/elevation card -> greedy rectangle merge -> material-batched Godot meshes -> use for layout/collision/navigation/sockets/LOD with Blockbench identity modules layered on top.
...
- Pixel room runtime proof is now a candidate keep. The current Cantina proof emits walk masks, collision masks, 38 merged collision shapes, 12 named sockets, socket-to-walk-cell resolution, and grid-routed actor probes from the same semantic cards.
