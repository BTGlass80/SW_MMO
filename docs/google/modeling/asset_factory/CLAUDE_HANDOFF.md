...
74. `VENGI_PIXEL_CARD_EVAL_PASS.md`
75. `generated/vengi_pixel_card_eval_v0/REVIEW.md`
76. `requests/README.md`
77. `requests/REQUEST_TEMPLATE.md`
78. `requests/ANIMATION_REQUEST_TEMPLATE.md`
79. `requests/FEEDBACK_TEMPLATE.md`
80. `codex_skill_draft/SKILL.md`
...
Read `PIPELINE_DECISION_LOG.md` before changing tool lanes. The current locked default is Blockbench/Blender/glTF/Godot proof for foreground identity assets, with Godot retained as the locked lane for terrain blockouts, review scenes, camera proofs, and tactical overlays. This is deliberately not "Blockbench for everything."
...
The Godot pixel-extrude proof is now available at `generated/godot_pixel_extrude_v0/REVIEW.md`. Treat it as a candidate keep for strict voxel props and tactical tokens. It proves a zero-credit lane where Codex/Godot reads original 2D pixel source cards and emits true cube-grid MeshInstance3D geometry. Same-color run merge reduced the blaster from 146 cubes to 32 and the ship token from 322 cubes to 94 while keeping the silhouette. Use it for pickups, signs, datapads, terminals, decals-with-depth, faction badges, and isometric ship tokens. Do not use it yet for full humanoids, large buildings, or terrain chunks.
...
The pixel Cantina runtime proof is now available at `generated/godot_pixel_cantina_runtime_v1/REVIEW.md`. Treat it as a candidate keep and the current proof that semantic pixel room cards can do more than visuals. It emits walk masks, collision masks, 38 merged collision shapes, 12 named sockets, socket-to-walk-cell resolution, and grid-routed actor probes from the same card grammar. The next best step is a reusable adapter that reads external cards/JSON, then a second SW_MUSH Cantina room to prove repeatability.
