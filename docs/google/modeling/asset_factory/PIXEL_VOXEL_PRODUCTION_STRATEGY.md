...
5. What runtime affordances should come out with it?
```
...
2. Make the Cantina runtime generator read external PNG/JSON cards instead of hard-coded grids.
3. Run the same room-card runtime pipeline on a second SW_MUSH Cantina room.
4. Add actor seat/standing anchors as explicit socket types and prove social placement.
