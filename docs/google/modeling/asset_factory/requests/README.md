...
```text
request -> grammar card -> Blockbench .bbmodel -> Blender GLB -> Godot/Blender preview -> validation -> verdict
```
...
```text
request -> semantic floor/detail cards -> Godot geometry/collision/socket proof -> Blockbench identity modules only where needed -> capture -> verdict
```
