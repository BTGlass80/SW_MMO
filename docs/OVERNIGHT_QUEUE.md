...
## Log (newest first)
- 2026-07-03 (Fable, tick 6): B7 PVP-CONSENT WIRED (DIV-0022) `01bc457`+`1d7926d` â€” duels
  ...
  zone-leave abort in the first commit â€” fixed + re-verified PASS (attack paths traced dead-end).
  RPC surface 45â†’57, gate 124 smokes. Also: thanu probe = no change needed (bars pass);
...
- 2026-07-03 (Fable, tick 5): ammo HUD `60d91f5` (client readout + low/dry tint + reload/dry
  ...
  CRASH DRILL PASS (kill â†’ journaled â†’ 3s restart â†’ same world restored â†’ client reconnects;
  stop.flag exits clean). 30-MIN 20-BOT DRESS-REHEARSAL SOAK PASS (60/60 samples, 20/20
  connected, 0 errors, server WS flat ~15.6 MB). First faucets-vs-sinks tally run â€” instrument
