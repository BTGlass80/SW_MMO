# Wave G Backlog â€” from the Fable external review (2026-07-02)

**Source:** an external code+design review by Fable, run against the review snapshot
`SW_MMO_Prototype_review_20260702_221957.zip` (post-Wave-F: S0â€“S19 + DIV-0019 PvP wired).
...
[`docs/fable/`](fable/):
- `SW_MMO_review_fable_2026-07-02.md` â€” the review (P0/P1/P2 + Wave-G queue G1â€“G9).
- `SW_MMO_livefire_balance_probe_2026-07-02.md` â€” live-fire + balance numbers (queue G10â€“G12).
- `SW_MMO_path_forward_2026-07-02.md` â€” strategy addendum (PT1, posture, process).
- `balance_probe.gd` â€” the probe tool, now relocated to [`tools/balance_probe.gd`](../tools/balance_probe.gd).
- `SW_MMO_waveG_delta_review_2026-07-03.md` â€” the DELTA review of the Wave-G night (scorecard:
  G1â€“G5/G7/G11/telemetry shipped & verified; G10 skipped â†’ **G13**; a new wounded_twice âˆ’2D phantom â†’
  **G14**; G12 tuning fails the probe â†’ **G15**; G6 â†’ **G16**; posture â†’ **G17**; faucets/sinks â†’ **G18**).
  The G13â€“G18 queue is folded into house format at the bottom of this file (**Â§ Delta follow-ups**).

This file distills that into the project's house backlog format so the items are tracked and
executable. **House rules still apply** (`docs/SESSION_HANDOFF.md` Â§Â§3â€“4): `[PAR]` pure-model+test
...
> live.** Those are G1â€“G3 (the P0s). Do them before building more breadth on top of Wave F.

---

## P0 â€” reconcile BEFORE building further on Wave F

### G1 â€” PvP death tiering â€” **DECIDED: true tiering (fork A), owner 2026-07-03**  `[HOT]` (depends on G2)
**What:** `pvp_rules_model.PVP_DEATH_SEVERITY = 5` + `is_kill(sev>=5)` and the two
`pvp_rules_model_smoke` asserts ("sev 3/4 is 'out', not dead") describe tiered death â€” but the live
...
`_handle_player_death`. `is_kill` is never called on the live path â€” **dead code whose smoke asserts
the opposite of shipped behavior.** A sev-3 PvP hit kills+respawns today.
**Why:** a green test encoding unwired semantics is the worst phantom â€” it has a CI badge. Pick one
...
**sev 5 = death; sev 3â€“4 = downed-in-the-field** (medic-relevant, not an instant kill); fix the two
`pvp_rules_model_smoke` asserts + the DIV-0019 "casualties" sentence to match. **Mandatory escape-hatch
bundle** (without it a downed lawless player with no friendly medic is *softlocked* â€” DIV-0012 excludes
...
command for a downed (sev 3) player. **Build as one seam with G2, `escalate()` FIRST** â€” under
`escalate()` most PvE/PvP outs arrive as *incapacitated-by-accumulation* (probe Â§D: 258/300 vs 197/300),
which is exactly the "downed, medic-relevant" state this tiering keys on, so G2's distributions inform
G1's tuning. This also applies to PvE death (DIV-0006) â€” reconcile both so a downed PvE player isn't
softlocked either.
...
### G2 â€” Wire `wound_ladder_model.escalate()` into live accumulation  `[HOT]` (+ `[PAR]` re-seed)
**What:** `escalate()` (cumulative WEG ladder: Wounded+Wounded â†’ Incapacitated, etc.) is complete,
correct, and tested â€” but **never called live**. Every live accumulation site uses `maxi(old, new)`:
`ground_combat_model.resolve_exchange`, `_resolve_return_fire`, and the DIV-0019 PvP defender
write-back. Consequence now that lethality shipped: **attrition cannot kill** â€” Wounded+Wounded stays
Wounded forever; death needs a single margin-16+ hit.
**Why (sharpened by the probe, Â§D):** against real monsters the single-hit regime dominates, so
`escalate()` barely moves *monster* lethality â€” **but near-peer PvP is where it matters**: under
...
strings** (`escalate(level, incoming_severity)` â†’ `level_index`), never raw ints; persist `wound_state`
...
**Alternative (if not wiring):** re-ledger explicitly â€” "maxi accumulation is the accepted MMO model;
one-shot-only death is intended" â€” and re-tune hostile damage knowing it. (Design G2 and G1 as **one
...
### G3 â€” PvP defenders cannot dodge (the WEG reaction layer is absent in PvP only)  `[HOT]`
...
victim's cover is read from their *intent* (absent â†’ 0), not persistent state. Net: against another
player's attack your `dodge`/`full_dodge` and cover do **nothing** â€” defense is armor + Strength soak
...
fidelity hole in the PvP slice â€” and it's not in DIV-0019's follow-up list.
...
reused across a window â€” built for exactly this. Read defender cover from persistent state
...
**Verify:** smoke â€” declared-dodge defender raises attacker's effective difficulty; full-dodge defender
skips their own attack AND applies vs incoming; two-process PvP check.

---

## P1 â€” will bite the next wave

### G4 â€” Hostiles never initiate  `[HOT]`
**What:** hostiles only fire as **return fire inside a player-initiated exchange**. A player who never
presses fire stands beside a Dune Sea spawn unharmed â€” lawless zones are dangerous only to volunteers,
...
gating.
**Verify:** two-process â€” an idle bot in `tatooine.dune_sea` takes fire and can die.

### G5 â€” Economy arbitrage floor guard  `[PAR]`
**What:** `MAX_TOTAL_DISCOUNT = 0.65` â‡’ buy floor = 0.35Ã—list, while `SELL_RATE = 0.40` â‡’ sell =
0.40Ã—list. **Floor < sell = a buyâ†’sell money printer** when stacked discounts reach the floor.
Unreachable today (bargain caps at 50%, needs ~17D) but one dial-turn away.
**How:** enforce `MAX_TOTAL_DISCOUNT <= 1.0 - SELL_RATE - Îµ` (or compute sell from *paid* price) + a
...
### G6 â€” Doc-rot / hygiene batch  `[PAR]`
...
- **P2-1 stale comment â†’ live phantom:** `ground_combat_model._wound_penalty_dice` comment says
  "sev 3â†’2, 4â†’2 (the fix)" but the delegate `penalty_dice_for_severity` returns **0** for 3/4
  (out-tiers moot). Fix the comment â€” it will mislead exactly the G2 session.
...
- **P2-3 README "Current Slice" unreadable:** the space paragraph is one ~600-word comma-chained
  sentence â€” the first thing a fresh session/human reads. Rewrite as short bullets; push detail into
  `docs/SPACE_SLICE.md`.
...
### G7 â€” Name policy: rename fixture + reserved-name filter  `[PAR]`
...
*(Note P1-2 â€” one-hostile-per-zone + forced engagement â€” is **accepted v1**; just ledger it and fold
multi-spawn + target selection into the positional-truth work, G9.)*

---

## Balance-probe-driven (live-fire findings, Â§Â§Aâ€“F of the probe doc)

### G10 â€” De-fang the dummy faucet + fix fallback/window-counter  `[HOT]`
...
take the **same reward branch as real kills** â€” `COMBAT_CP_REWARD` + zone influence + territory
...
disable â€” corrupts future replay/ordering tooling.
**How:** hostile-death fallback â†’ **no target / hold fire** (not the global dummy); dummy disables pay
...
### G11 â€” Resolved-pool content smoke + fix prose stat strings  `[PAR]`
...
missing skill wiring** (listed atk never reaches the resolver â†’ silent 3D; `merdeth`/`acklay` resolve
*above* their listed value â€” data and resolver disagree silently).
**How:** extend `content_smoke` to assert, for every `hostile:true` creature, that resolved
`target_damage_pool` and `target_attack_pool` are **â‰¥ 1D** and (where a listed stat exists) **match**
it â€” turns the whole rot class into a gate failure. Fix `glim_worm`/`mip_swarm`/`spor_crawler` to
machine dice codes; reconcile listed-vs-resolved attack stats. The probe's Â§B block is a ready template.
**Files:** `data/creatures_clone_wars.json`, `scripts/tests/content_smoke.gd`.

### G12 â€” `threat_tier` + alert-banded spawn table + loot-by-tier  `[PAR]` (probe = acceptance)
**What (the threat cliff, Â§C/Â§E):** the ambient spawn table spans **free ATM â†’ 20-second death â†’
instant execution â†’ invulnerable wall**, chosen by zone-alert bias, so a fresh player entering the Dune
Sea rolls dice on *which economy they entered* â€” the single biggest PT1 feel risk. And **risk/reward
...
*replaces* the already-low range-10 difficulty; cover is marginal vs 6â€“8D pools) â€” survivability comes
from **tier selection, not micro**.
**How:** add a `threat_tier` field per creature; **band the spawn table by zone alert** (calm lawless =
tiers 1â€“2 only; escalation unlocks upper tiers); treat `merdeth`-class as **event/boss** content, never
ambient. Make **loot tier per creature** (band Ã— threat_tier) so reward tracks risk. Re-run
...
*(Player-guide note to add with this: survivability = tier selection, not tactics â€” there is no
tactical escape hatch from an up-tier spawn.)*

---

## Owner forks â€” DO NOT auto-build (surface for a decision)

1. âœ… **DECIDED 2026-07-03 â€” PvP death tiering = fork A (true tiering).** sev 5 = death; sev 3â€“4 =
...
2. âœ… **DECIDED 2026-07-03 â€” two-games relationship = both (option C), provisional ("see how they turn
...
3. âœ… **DECIDED 2026-07-03 â€” launch posture adopted.** *"The MMO ships thin and iterates live; the MUSH
...
4. **First Strangers Night (PT1)** â€” âš™ï¸ **PREP APPROVED 2026-07-03 (owner, attended): build the PT1 prep
   track** (G8 auth bundle + server watchdog + 20-bot soak + envelope replay tool) after G14â€“G18 land, so
   PT1 is schedulable whenever the owner picks a date. **The date itself stays owner-gated.** The evening:
   5â€“10 outside players running a scripted ~30-min loop (chargen â†’ range â†’ travel â†’ Dune Sea fight â†’
   die/respawn â†’ insure â†’ buy/sell â†’ lawless duel â†’ org claim). Let PT1 *pull* priorities.
5. **Auth/crypto bundle (G8):** âœ… **UN-GATED 2026-07-03** as part of the PT1 prep ruling. `check_secret`
   stores/compares secrets in **plaintext** JSON over **unencrypted ENet**; unsecured accounts are
   first-claimer-wins. Godot ships `Crypto`/`HashingContext` â€” salted hash at rest is a small slice; DTLS
   (or an explicit "dev transport" banner) for flight. Queued in Â§ Delta follow-ups execution order.
6. **Positional-truth spike (G9) â€” the next fidelity cliff.** Distance is a profile constant
   ...
   radius) â€” the single biggest step from "action windows bolted to a lobby" toward the
   `REALTIME_D6_TRANSLATION.md` thesis. Design doc first (`world-sim-designer`), then slices.
7. **LLM policy â€” author-time, never runtime (resolves the parked fork with a *shape*):** keep the
   deterministic Director; if AI flavor is wanted (NPC barks, event headlines, quest text), generate
   **batches offline** that ship as reviewed JSON â€” the MUSH questline authoring pattern. "Yes at the
...
- **Telemetry before tuning** â€” one structured JSONL server log (death: killer/zone/severity-path;
...
- **Seam audit as a rule, not a favor** â€” after any wave touching `network_manager`/`net_world`/
  `combat_arena`, one mandatory read-only tick by a fresh agent hunting docâ†”modelâ†”wire disagreements
  only (this review was that pass; P0-1 is why convention alone wasn't enough).
- **Dead-symbol detector in the gate** â€” for each public func/const in `scripts/rules`+`scripts/net`,
...
- **`mush-content-porter` re-extraction cadence** â€” the 06-24 quest/data port is ~10 heavy MUSH
  ...
  (giver â†’ skill checks â†’ combat â†’ return, funnel-routed rewards) when wiring DIV-0020 â€” don't invent a
  parallel objective grammar.
- **Nearly-free wins:** (a) **envelope replay tool** â€” envelopes already carry exchange seeds; a dev
  command that re-runs `resolve_exchange` from a pasted envelope gives combat debugging + player-dispute
  resolution for free. (b) **server watchdog** â€” auto-restart + the existing crash-safe persistence =
  a survivable PT1 evening. (c) **20-bot headless soak test** â€” the client affordances (`--autofire`,
...
## Key balance numbers worth keeping (from the live probe â€” see the probe doc for full tables)

- **Threat cliff (green profile, quickstart human), P(taken out)/window Â· median time-to-out Â· farm rate:**
  `hitcher_crab` 1.35% Â· ~3.2 min Â· **~195 cr/min** Â· `tusken_warrior` 13.3% Â· **~20 s** Â· ~194 cr/min Â·
  `acklay` 63.7% Â· 1 window Â· ~37 cr/min Â· `merdeth` 95.9% (82.5% killed outright) Â· 1 window Â· ~1 cr/min.
- **`escalate()` vs `maxi()`** (hitcher_crab/green): median windows-to-out 38 â†’ **22 (âˆ’42%)**; but
  against high-tier monsters the single-hit regime dominates and it barely moves â€” **the win is in
  near-peer PvP** (see G2).
- **Economy:** loot keys on scale only, so risk and reward are **anti-correlated** at the top (crab
  dominates); heavy blaster (750) â‰ˆ 4 min of safe crab farming; insurance â‰ˆ break-even on the starter
...
*(Historic â€” the G1â€“G12 wave shipped 2026-07-03; see the delta-review scorecard. Current order lives
in Â§ Delta follow-ups below.)*

1. **G2 â†’ G1, plus G3** (the P0 seam): `escalate()` (G2) FIRST, then G1 true-tiering + the escape-hatch
   bundle (they're one seam); G3 (PvP defender dodge) in parallel.
2. **G10, G11** (de-fang the faucet + stop silent data rot â€” both cheap, both distort every other number).
3. **G4** (hostile initiation â€” makes lawless actually dangerous), **G5** (floor guard), **G6/G7** (doc + name hygiene).
...
*Do not wire `tools/balance_probe.gd` into the gate â€” it's statistical and slow; run it on demand as the
acceptance instrument for the spawn-table / loot-tier / `escalate()` drops.*

---

## Delta follow-ups (Fable delta review 2026-07-03) â€” G13â€“G18

**Status of the original queue** (delta-review scorecard, verified at symbol level + live): G1 âœ… (DIV-0027
downed/tiering, live-verified) Â· G2 âœ… (level-string seam; its one caveat became **G14**) Â· G3 âœ… (defender
dodge) Â· G4 âœ… (unprovoked aggression, live-verified) Â· G5 âœ… (floor guard, structural) Â· G7 âœ… (name policy,
whole-token) Â· G10 â†’ shipped later as **G13** Â· G11 âœ… (data rot + resolved-pool smoke) Â· G12 âš ï¸ mechanism âœ“
(DIV-0028) / tuning âœ— â†’ **G15** Â· G6 âŒ â†’ **G16** Â· telemetry âœ… (JSONL, character_id join).

### G13 â€” de-fang the dummy faucet (= the deferred G10) `[HOT]` â€” **DONE `8cd9619` + LIVE-VERIFIED 2026-07-03**
Dummy `target_down` pays capped CP only (no zone/territory influence, no Force `disables` signal);
hostile-death fallback â†’ `HOLD_TARGET` sentinel, never the cross-zone dummy (dummy only in the secured
...
**Live acceptance (this session):** lawless autofire bot â€” 3 dummy hits in the secured spawn zone only
...
### G14 â€” wounded_twice âˆ’2D made LIVE `[PAR]` â€” **DONE (merged `92e255d`+`6c01b62`, adversarially verified)**
Level-string penalty plumbing at EVERY live derivation site: `resolve_exchange` (player+target),
`_resolve_full_dodge_exchange`, `_resolve_return_fire` (both sides), the G3 PvP defender path â€” **plus two
the adversarial verifier forced**: the G4 incoming-fire path (`resolve_incoming_fire_window`, a wounded_twice
victim of unprovoked hostile fire now dodges at âˆ’2D) and the **heal/recovery seams** (`_live_wound_state`
...
string is load-bearing (severity int = 2 in every row; only the level moves the penalty 1â†’2). Three lying
comments fixed; DIV-0008 updated. **Verifier advisories (follow-up notes):** (a) `resolve_hostile_aggression`
only refreshes `player_wound_level` when severity strictly advances â€” an equal-severity unprovoked hit doesn't
escalate woundedâ†’wounded_twice on that path (the PvP defender path does); fold into the DIV-0008 escalate
follow-up. (b) solo-space `_crew_wound_penalty_dice` is severity-int only â€” fine while space is parked; needs
the same sweep if space ever goes live. (c) no end-to-end NetworkManager First-Aid-at-14 test yet.

### G15 â€” G12 re-tune, probe-accepted `[PAR]` â€” **DONE (merged `5ef7ae2`+`980ab45`, adversarially verified)**
All of delta Â§4 (aâ€“e) landed with the probe RUN as acceptance (full table in the merge commit `107b7bc`):
20 tier changes from measured lethality (acklay/mutant_acklay â†’ t4, out of default ambient); merdeth/
krayt_dragon/rancor = t5 `boss:true`, never ambient; ONE loot axis (scale gone from credits);
`LOOT_TIER_MULT {1,1,3,8,10}` â†’ per-tier mean cr/min monotone (185.6 â†’ 251.4 â†’ 416.0); unknown-alert
clamps to t2 fail-safe. The verifier's blocking find â€” the "boss channel" was a documented phantom that
silently killed `q_krayt_bounty`/`q_rancor_sighting` â€” was fixed by LANDING the channel: quest-driven
...
(bystanders exposed â€” acceptable in lethal zones, or a future per-player instance); an active boss bounty
...
non-monotone within tiers (svaper t4 824 cr/min is the outlier â€” next tuning pass); `_forced_spawn`
(test-only) doesn't carry threat_tier.

### G16 â€” the G6 doc-hygiene tick, for real `[PAR]`
Gate **prints** smoke + RPC counts (docs say "see gate output" â€” kills the 59/66/72/111 drift class);
DIV-0011 `force_sensitive` storage-divergence sentence; 3 `d6_rules` house-rule header/ledger notes;
README "Current Slice" â†’ bullets; pip-drop convention note. **Add (found this session):** SESSION_HANDOFF
Â§6 documents a `--quit-after` client flag that does not exist â€” fix the affordance list there.

### G17 â€” posture codification `[owner âœš PAR]` â€” **RULED 2026-07-03 (owner, attended): models-OK / wiring-parked**
...
`siege_*`/`city_*`/server-side `space_*` files in `scripts/net/*` or RPC surface â†’ audit failure).

### G18 â€” faucets-and-sinks invariant `[PAR]`
...
1. ~~G13~~ DONE + live-verified (`8cd9619`). ~~G16 + G17 + G18~~ **DONE `1983b34`** (gate prints
   smoke/RPC counts; not-before-live invariant is mechanical â€” and found `siege_state_model.gd`
...
2. ~~G14 + G15~~ **DONE â€” merged serially (`bf27f1e`, `107b7bc`), gate green each, claim released.**
3. ~~PT1 prep track~~ **DONE 2026-07-03, claim RELEASED.** G8 auth bundle `6d2834e` (salted-hash
   ...
   upgrade; DTLS still out of scope â€” gates the first non-LAN evening). Envelope replay tool
   ...
   honesty fix). **Live soak acceptance PASS twice** (20 bots Ã— 7 min and 6 bots Ã— 2 min: all
   connected, zero SCRIPT ERROR, zero engine errors, server survived; ~128 MB/client, server WS
   settles ~10 MB).

### Next queue (unblocked, non-owner-gated â€” for the loop)
- ~~replay_inputs server-side attach~~ **DONE `5c7da43`**: broadcast byte-identical (smoke-proven);
  ...
  envelope replayed through `tools/envelope_replay.gd` â†’ REPRODUCED. Incoming-fire envelopes got
...
- ~~First-Aid-at-14 flow smoke~~ **DONE `5c7da43`** (`first_aid_at_fourteen_smoke`: live
  wounded_twice â†’ difficulty 14, heals to wounded, no collapse on write-back, follow-up at 11).
- ~~equal-severity escalation asymmetry~~ **DONE `5c7da43`** (unprovoked wounded+wounded â†’
  wounded_twice, level-string load-bearing; DIV-0008 note updated; `hostile_aggression_smoke` Â§H).
- ~~svaper cr/min outlier~~ **DONE `f86979f`** (per-creature `loot_mult` dial inside the single
  axis; t4 spread 7.87x â†’ 1.77x, per-tier means stay monotone, probe table in the merge message).
  Residual: t3 thanu 366.5 cr/min is the next within-tier high earner (passes the bar).
- ~~ammo recurring sink (old B8)~~ **DONE `14ed176` (DIV-0029)**: latent WEG `ammo` fields live â€”
  ...
  packs + lazy veteran migration; "you"-block ammo readout; reload telemetry (informational â€”
  the pack BUY is the counted sink, no double-count). Two-process proven end-to-end (6-mag â†’
  two auto-reloads â†’ refusal; dummy run = 0 ammo lines). Analytic economics in the row: sink
  3 cr/min (100-mag) to 60 cr/min (5-mag disruptor) vs ~197 cr/min t2 income â€” never negative.
- ~~MUSH weekly re-extraction cadence â€” manifest half~~ **DONE `7429dfd`**:
...
two-process: full 6-mag lifecycle visible client-side). ~~watchdog crash drill~~ **PASS** â€”
...
without restart. ~~30-min 20-bot dress-rehearsal soak~~ **PASS** â€” 60/60 samples, 20/20
...
â€” instrument works end-to-end (character_id join + reload/buy events); flows tiny & skewed
(autowalk bots don't fight; dummy is free by design) â†’ the real economy read needs PT1 humans.

Tick 6 results (2026-07-03 evening):
- ~~B7 PvP-consent duels + bounties~~ **WIRED (DIV-0022): `01bc457` + audit-fix `1d7926d`.**
  Consent composes OVER the DIV-0019 zone gate (Â§1.2 precedence verified); duels work in ANY
  zone once accepted (sev-3 KO clamp â€” three clamps coexist strictly ordered: sparring 2 <
  ...
  the first commit** (challenger self-accept consent bypass; phantom zone-leave abort) â€” both
  fixed + re-verified PASS with the attack paths traced dead-end. Two-client live proofs:
  secured-zone duel â†’ KO â†’ no death penalty; non-duel secured fire rejected Ã—77; bounty pays
...
- Weekly `mush_sync_manifest.py diff` (next due ~2026-07-10; re-port slice on drift â€” MERGE
...
- **Before PT1 (owner picks the date):** a full 20-bot Ã— 30-min evening soak under
